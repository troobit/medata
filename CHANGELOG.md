...
### Fixed (Bugfix spec — lidar-plane-fit-degenerate-on-clean-capture)

Fixes the "Unable to detect a flat surface. Please place the meal on a level surface." modal firing on every shutter tap on iPhone 13 Pro Max iOS 26.5 even with both stages succeeding at 1920×1440 and live LiDAR coverage at 87.5%. Root cause: `Pipeline.fitSupportPlane` constructed an all-ones `roughMask` so `LiDARPlaneFitter.collectCandidatePoints`'s lower-edge scan band collapsed to a single fully-food row and returned zero candidate points, throwing `SupportPlaneError.noLidarPoints` → `EstimationFailure.lidarFitDegenerate` → the modal. Resolved by replacing the all-ones mask with a centred-rectangle approximation at `fillFraction = 0.7` (Decision 1) so the band sits on visible table pixels.

- `MedataCore/Sources/Pipeline/CentreRectangleMask.swift` (new) — `makeCentreRectangleMask(width:height:fillFraction:)` helper plus the `centreRectangleFillFraction: Float = 0.7` named constant. The fill fraction is calibrated to the iPhone 13 Pro Max main wide camera (~67° HFOV) at the gating distance (~30 cm).
- `MedataCore/Sources/Pipeline/Pipeline.swift` — `fitSupportPlane` builds the rough mask through `makeCentreRectangleMask` rather than `BinaryMask(pixels: [UInt8](repeating: 1, ...))`. Adds DEBUG-only `event=supportplane.start` / `event=supportplane.end` structured-log entries (Logger subsystem `ie.medata.app`, category `Shutter` — same channel as `estimate.start` / `estimate.end`) carrying mask dimensions, fillFraction, residual_mm, inlier count, candidate-point count, and `SupportPlaneError` case on failure.
- `MedataCore/Sources/SupportPlane/LiDARPlaneFitter.swift` — DEBUG-only `debugLastCandidatePointCount` / `debugLastInlierCount` static counters populated inside `fit(_:)` so the Pipeline-side log can surface a numeric trace without changing the production API.
- `MedataCore/Tests/PipelineTests/CentreRectangleMaskTests.swift` (new) — Swift Testing suite pinning the helper's pixel layout: the `[2,8) × [2,8)` inner rectangle for `10×10` at `fillFraction = 0.7`, the buffer-length invariant, `fillFraction = 1.0` fills everything, the 0.5 fraction shrinks symmetrically, and the `centreRectangleFillFraction` constant is `0.7`.
- `MedataCore/Tests/PipelineTests/SupportPlaneRoughMaskTests.swift` (new) — Swift Testing regression suite over a shared synthetic depth fixture: an all-ones mask throws `SupportPlaneError.noLidarPoints` (sentinel for the pre-fix behaviour); the centre-rectangle mask returns a finite plane within `gravityAngleMaxRad` of gravity with residual below `LiDARPlaneFitter.residualMaxMm`.
- `Package.swift` — added `SupportPlane` and `CaptureKit` to the `PipelineTests` target dependencies so the regression suite can build synthetic `DepthMap` / `BinaryMask` fixtures and call `LiDARPlaneFitter.fit` directly.

### Fixed (Bugfix spec — lidar-plane-fit-oom-on-device-1920x1440)

Fixes a `Fatal error: failed to allocate 32198713632 bytes of memory with alignment 8` abort on iPhone 13 Pro Max iOS 26.5 that surfaced immediately after the new DEBUG `event=supportplane.start width=1920 height=1440 fillFraction=0.7` log line (i.e., once the prior bugfix unblocked the `noLidarPoints` early return). Root cause: `LiDARPlaneFitter.refine` called `LinearAlgebra.svdFull(_, rows: 3, cols: n)` on the centred inlier matrix; `svdFull` requests JOBVT='A' and always allocates an n×n Float V^T (`vt = [Float](repeating: 0, count: n*n)`), which at the ~89,720 inliers produced by a 1920×1440 frame plus the 0.7-fillFraction centre-rectangle mask is ~32 GB. `refine` never reads `svd.vt`. Resolved by replacing the 3×n SVD with a SVD of the 3×3 symmetric scatter matrix `M = Σᵢ (pᵢ − c)(pᵢ − c)ᵀ` (Decision 1) — algebraically equivalent (M's eigenvectors are A's left singular vectors; M's singular values are A's squared) and bounded by O(n) accumulator work plus a constant-time 3×3 SVD.
