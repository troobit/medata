...
public enum AccuracyHarness {
    ...
    static func perClassStats(meals: [MealEvalInput]) -> [String: ClassAccuracyStats] {
        ...
        for c in classes {
            ...
            // Per-class ground-truth: use proportional share when total is known.
            // For simplicity use per-class predicted vs per-class actual (not available directly
            // in MealEvalInput). We compute per-class MAE as |predicted_c - 0| which is not
            // meaningful without per-class ground truth. AccuracyHarness works at meal-total level;
            // per-class breakdown reports predicted distribution and status only.
...
// Same implementation as in SupportPlane/Hash.swift; copied here to avoid a dependency.
