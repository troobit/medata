...
- `MedataCore/Sources/GlucoseWidgetShared/` — the app↔widget-extension contract
  (specs/ui/glucose-lock-widget): snapshot DTO, `GlucoseTrend`/`GlucoseBandStatus`,
  App Group + widget-kind ids, the UserDefaults snapshot store, and the pure
  staleness ladder. Exposed as its OWN `.library` product, not through the
  `MedataCore` umbrella, so the extension's link closure cannot pick up GRDB
  (Decision 10). Two boundaries hold it, both because `Persistence` depends on
  it and so anything imported here reaches `Pipeline` and the macOS
  `HarnessCLI`: the dependency list must stay **empty** (asserted by the
  dump-package check in `GlucoseWidgetSharedTests/PackageBoundaryTests`), and it
  must never `import WidgetKit` (Decision 12 — the dump-package check is blind
  to framework imports, so that half is held by review).
- `MedataCore/Sources/PortableContracts/Schemas/*.proto` — 28 canonical schemas
  (Decision 31; CorrectionRecord.proto added by ui/meal-review). `Schemas/` is
  `exclude`'d from the SwiftPM target.
