...
> fixture scripts named in §§3–5 are not yet written — each is flagged **(to
> write)** at its command.
...
Scripts marked **(exists)** are in the repo today. Scripts marked **(to write)**
do not exist yet — author them per the description in that section, then run the
command shown. As of this writing only `tools/segmenter/export.py` and the
`HarnessCLI` benches exist; the dataset/training/fixture scripts are yours to
write.
...
python tools/segmenter/build_class_mapping.py \
    ...
    --out tools/segmenter/class_mapping_foodseg103_v1.json   # (to write)
```
...
python tools/segmenter/prepare_dataset.py \
    ...
    --heldout-frac 0.12 --seed 1234                          # (to write)
```
...
python tools/segmenter/train.py \
    ...
    --out tools/segmenter/build/checkpoint.pt                # (to write)
```
...
python tools/segmenter/make_fixtures.py \
    ...
    --out tests/fixtures/segmenter/heldout                   # (to write)
