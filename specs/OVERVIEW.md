...
| [Insulin Dosing](#insulin-dosing) | data | 2026-08-14 | In Progress | full | An on-device guess at the bolus dose for a meal just estimated, from a time-banded carbohydrate ratio stored in exactly one canonical direction — `g/U`, grams of carbohydrate covered by one unit — with the reciprocal rendered read-only beside it and never stored. Seeds overnight 10.0, breakfast 5.0, lunch 10.0, dinner 10.0 g/U, reproducing the developer's rule of 2 U per 10 g in the morning falling to 1 U per 10 g later. Fat and protein are recorded on every suggestion and change no number. Narrowly reverses [Regression Suggestion Integration](#regression-suggestion-integration)'s dose-suggestion non-goal (Decision 1): suggestion arithmetic and insulin-on-board move on-device, parameter fitting stays in `~/repos/medreg`. 14 of 30 tasks done: the pure `Dosing` target (ratio, bands, insulin-on-board, suggester) and the `dose_suggestions` ledger at schema 8. The App layer exists only as three tagged attempts (`insulin-dosing-ui-attempt-1/2/3`) awaiting a device look. |
