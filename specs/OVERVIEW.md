...
| [Glucose Lock Screen Widget](#glucose-lock-screen-widget) | ui | 2026-07-27 | Planned | full | WidgetKit accessory widget (circular/rectangular/inline + StandBy) showing the most-recent `bsl` reading + a derived trend arrow, fed by an app-written App Group snapshot; status is a non-colour channel that survives the Lock Screen's monochrome rendering, with a dim>15/hide>30 staleness ladder and a `medata://graph` tap target. 13 tasks planned. |
