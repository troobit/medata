...
| [CGM Direct](#cgm-direct) | data | 2026-08-17 | Planned | full | Direct-from-device sibling of CGM Connect: a second BLE central rides the worn Libre 3+'s Abbott session and turns each ~1-minute connection/notify event into a reliable background wake for the existing rate-gated LibreLinkUp fetch — bounded ~6-minute worst-case staleness in place of an unbounded OS wake deferral, no glucose value of its own and no new ban exposure (Phase A). Phase B (on-device AES decrypt, zero network) stays research-gated on the living feasibility note. 0 of 11 tasks; stream 2 carries the standing re-verification sweep and the user-controlled uploader routes (region-switch LibreLink, Juggluco bridge). |
