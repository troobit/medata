...
| [Alternative Class Candidates](#alternative-class-candidates) | estimation | 2026-08-10 | 0/16 | full | Retain the per-detected-food alternative classes the segmenter computes and discards, so a relabel shortlist can offer a food the user has never chosen. Owed by meal-review Decision 18, which deferred it and shipped recency ordering. Recency is kept as a live component of the ordering rather than superseded (Decision 6). Requirements and design both approved 2026-08-10 (design-critic + peer review applied; Decisions 7–12: three-layer shortlist, JSON-sized encoding, shared-function replay parity, pre-implementation statistic probe, 64-sample floor, stated Req 8 confound). Task file written 2026-08-11: 16 executable tasks in six phases (streams 1 and 2) plus a separate `Acceptance gates` phase of 3 STOP items that no autonomous run may attempt — the device latency check, the Decision 5 verdict against recency, and the conditional removal a negative verdict obliges. Only the Decision 11 probe is ready; everything else is blocked behind its verdict. |
...
| [Glucose Lock Screen Widget](#glucose-lock-screen-widget) | ui | 2026-07-27 | In Progress | full | WidgetKit accessory widget (circular/rectangular/inline + StandBy) showing the most-recent `bsl` reading + a derived trend arrow, fed by an app-written App Group snapshot; status is a non-colour channel that survives the Lock Screen's monochrome rendering, with a dim>15/hide>30 staleness ladder and a `medata://graph` tap target. 13 of 15 tasks done — the MedataCore contract and pure maths, the app-side publisher and `medata://graph` route, the App Group + extension wiring, the widget kind and its per-family views, the agent notes, and the build/test/spell verification sweep are in; the human-gated device pass (14) and the trend-derivation tightening against observed CGM cadence (15) remain. |
| [Meal Review](#meal-review) | ui | 2026-08-06 | In Progress | full | Collapse the read-only segmentation review and the result screen into one post-capture surface: detected foods outlined on the photo, relabel and reject per food, serving/gram amounts, and one tap to record. The deliverable is the corpus — one correction record per detected food per capture, predicted values beside corrected ones, never discarded on age, count, or any sweep. 21 of 22 tasks done — contracts and shared β re-derivation, the exempt correction store (self-healing upsert), MealReviewView with contour outlines/relabel/reject/scale, the capture-stack rewire (SegmentationReviewView deleted, ResultView collapsed to the history read path), and the supersessions owed design-handoff-00 and serving-adjust; task 22's human-gated device sheet (prerequisites.md) is the only open item. |
