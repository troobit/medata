...
| [Insulin Dosing](#insulin-dosing) | data | 2026-08-14 | In Progress | full | An on-device guess at the bolus dose for a meal just estimated, from a time-banded carbohydrate ratio stored in exactly one canonical direction — `g/U`, grams of carbohydrate covered by one unit — with the reciprocal rendered read-only beside it and never stored. Seeds overnight 10.0, breakfast 5.0, lunch 10.0, dinner 10.0 g/U, reproducing the developer's rule of 2 U per 10 g in the morning falling to 1 U per 10 g later. Fat and protein are recorded on every suggestion and change no number. Narrowly reverses [Regression Suggestion Integration](#regression-suggestion-integration)'s dose-suggestion non-goal (Decision 1): suggestion arithmetic and insulin-on-board move on-device, parameter fitting stays in `~/repos/medreg`. 9 of 25 tasks done: the pure `Dosing` target (ratio, bands, insulin-on-board, suggester) and the `dose_suggestions` ledger at schema 8. The App layer exists only as three tagged attempts (`insulin-dosing-ui-attempt-1/2/3`) awaiting a device look. |
...
| [CGM Direct](#cgm-direct) | data | 2026-08-17 | In Progress | full | Direct-from-device sibling of CGM Connect: a second BLE central rides the worn Libre 3+'s Abbott session and turns each ~1-minute connection/notify event into a reliable background wake for the existing rate-gated LibreLinkUp fetch — bounded ~6-minute worst-case staleness in place of an unbounded OS wake deferral, no glucose value of its own and no new ban exposure (Phase A). Phase B (on-device AES decrypt, zero network) stays research-gated on the living feasibility note. 4 of 11 tasks; stream 2 carries the standing re-verification sweep and the user-controlled uploader routes (region-switch LibreLink, Juggluco bridge). |
