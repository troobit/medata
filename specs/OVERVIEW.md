...
| [MyFoodRepo Bridge](#myfoodrepo-bridge) | estimation | 2026-07-25 | In Progress | prd | Bridge the MyFoodRepo-273 dataset (24,119 images, CC BY 4.0) into the training corpus to fix the three zero-image staples, and add the required cereal class as palette v2 (36 channels) end-to-end: palette, food DBs, mappings, co-stats, training/export tooling. 10/22 tasks done across 4 contexts; dataset acquisition is blocked at the AIcrowd auth wall (human gate), with the retrain/validate/export chain queued behind it. |
...
PRD-lane spec: the contingent big move named by SNAQ Parity — bridge the AIcrowd Food Recognition Benchmark v0.4 (MyFoodRepo-273: 24,119 images, 39,325 COCO polygons, CC BY 4.0, menuCH ontology) into the training corpus, the only verified fix for the three zero-image staples (brown_rice, bread_wholemeal, potato_mashed — segmenter-foundation Decision 21), and land the missing cereal class flagged as a required MVP fix. Four contexts: palette v2 (cereal at solid index 24, 36 channels, `ClassPalette.v2Standard` + `PaletteMigrator`, cereal CoFID row baked into both DBs under the bake lock, MD-29 — `PipelineFactory` keeps binding v1 until a v2-trained model is promoted); the dataset bridge (acquisition → coverage audit → `class_mapping_myfoodrepo273_v1.json` → polygon rasterisation → merged corpus preserving the byte-identical 182-image leak-free anchor); training and export (staple-safe recipe only — `none`/`sqrt_inverse` weighting, promotion gated on beating the 0.3776 leak-free anchor without material staple regression); and specs/docs. 10/22 tasks done — palette v2 and the 36-channel tool chain (FoodSeg103 mapping, Recipe1M co-stats with cereal coverage, train/export/validation) are landed and green; dataset acquisition is blocked at the AIcrowd auth wall (login + accept challenge terms — steps in `data/myfoodrepo273/ACQUISITION.md`), with the mapping, merged corpus, retrain, validation, and export/deploy chain queued behind it.
