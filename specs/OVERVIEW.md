...
|------|--------|---------|--------|------|---------|
| [Research](#research) | estimation | 2026-05-06 | Done | full ·iterative | Low-compute on-device system estimating carbohydrate content from one or two iPhone photos. |
...
| [Minimum Viable Volume Estimator](#minimum-viable-volume-estimator) | estimation | 2026-06-23 | Superseded | smol | Retired before tasks/code (decision_log Decision 5): its premise that the dev-stub yields uncarveable masks proved false; the real two-view defect lives in `bugfixes/two-view-carve-no-volume`. |
...
| [Nutrition5k Calibration](#nutrition5k-calibration) | estimation | 2026-07-02 | Done | full | Bridge the Nutrition5k RGB-D dataset into the harness to fit per-class β bulk-correction factors, bake them into the food DB with lineage, report carb/protein/fat accuracy against a β=1.0 baseline, and add standalone-liquid classes to the in-place-redefined palette. All 45 tasks done (39 implementation + closeout consolidation); the post-checkpoint single-dominant re-fit stays gated on model-production Bucket C. |
...
| [Segmenter Foundation](#segmenter-foundation) | estimation | 2026-07-10 | Done | full | Re-derive the segmenter bars (gate 0.60 → mean IoU ≥ 0.48, staple floors 0.50 → 0.45), upgrade the training recipe (stronger init + co-occurrence loss, stratified heldout re-cut) as the primary lever, and gate a SegFormer-B0 backbone swap on a Core ML conversion spike measured on the hardware floor. The training chain executed with a NEGATIVE verdict (Decision 24): the co-occurrence recipe regressed the leak-free same-set mean 0.3776 → 0.3253 with four staple regressions, so `24e0b022241a` stays bundled; V2 init was separately rejected mid-run (Decision 23) and the floor re-based to the iPhone 16 Pro (Decision 22). Combined-loss fallback run verdict recorded (Decision 25: weighting is the culprit). The SegFormer-B0 spike is a FULL PASS (Decision 28, 2026-08-13): 12.68 ms median on the 16 Pro, all 315 dispatchable ops ANE-resident, zero fallback. ALL 23 tasks done: the conditional retrain (task 22) executed 2026-08-13 and returned a NEGATIVE verdict (Decision 30) — SegFormer-B0 read 0.2912 mean food-class IoU against the incumbent `ab812dc3aa9d`'s 0.3927 on the leak-free anchor (staples 0.2987 vs 0.3870) where adoption needed ≥ +0.02 on both, so `deeplab_mnv3` stays and Requirement 3 closes. The rejection is bounded to an equal step budget: the candidate was still improving at epoch 12 while the incumbent had plateaued. |
...
| [MyFoodRepo Bridge](#myfoodrepo-bridge) | estimation | 2026-07-25 | In Progress | prd | Palette v2 (cereal at 24, 36 channels) plus the dataset bridge that fixed the zero-image staples — MD-30 substituted the obtainable Food Recognition 2022 release for the unreachable v0.4, and the merged-corpus retrain beat the leak-free anchor (0.3927 vs 0.3776; cereal 0.48 on val) and shipped as model `ab812dc3aa9d` (Decision 27). 20/22 tasks done. The deploy and its lineage proof landed 2026-08-16 (nine capture rows carrying coreml_ab812dc3aa9d); remaining are the two human STOPs — the capture pass, run once but not yet confirmable, and the Xcode ANE residency check. |
...
| [Event Log Schema](#event-log-schema) | data | 2026-06-13 | Done | full | Uplift persistence to a long-form event log with fixed timestamp/event_type/value columns and JSON metadata. |
...
| [Insulin Dosing](#insulin-dosing) | data | 2026-08-14 | In Progress | full | An on-device guess at the bolus dose for a meal just estimated, from a time-banded carbohydrate ratio stored in exactly one canonical direction — `g/U`, grams of carbohydrate covered by one unit — with the reciprocal rendered read-only beside it and never stored. Seeds overnight 10.0, breakfast 5.0, lunch 10.0, dinner 10.0 g/U, reproducing the developer's rule of 2 U per 10 g in the morning falling to 1 U per 10 g later. Fat and protein are recorded on every suggestion and change no number. Narrowly reverses [Regression Suggestion Integration](#regression-suggestion-integration)'s dose-suggestion non-goal (Decision 1): suggestion arithmetic and insulin-on-board move on-device, parameter fitting stays in `~/repos/medreg`. 23 of 33 tasks done: the pure `Dosing` target (ratio, bands, insulin-on-board, suggester), the `dose_suggestions` ledger at schema 8, and — Decision 16, 2026-08-25 — the App layer as a merged synthesis: attempt 3's `LogSheet`/`EntryChrome` consolidation is the base, attempt 2's `DoseReadoutLine` reach is ported on top, the ledger write is live, and Req 6.10 extends the readout to the history surfaces as the recorded row's values. The on-device STOP and the evidence-gated phases remain. |
| [Activity Events](#activity-events) | data | 2026-08-14 | In Progress | full | `EventType.activity` as a first-class manual event so exercise becomes a recorded covariate long before HealthKit workouts are wired up: a stable machine kind key (swim, waterpolo, cycle, run, walk, gym, other) each carrying a fixed aerobic/anaerobic/mixed character, optional duration recorded as absent rather than zero, and `manual`/`healthkit` provenance so a later source needs no migration. Records the covariate and adjusts no dose (Decision 5). 8 of 9 tasks done: the store foundation, then the entry sheet, the deep link, the Graph band and the records row — each UI surface built as two tagged attempts for a device comparison. Only the on-device entry pass (task 8) is open. |
| [Dose Schedule](#dose-schedule) | data | 2026-08-14 | In Progress | full | A recurring-dose schedule — nominal 15 U at 07:30 and 19:30 — whose reminder persists until the dose is logged and whose logging costs one tap. Reverses insulin-dosing Req 12.7's no-prompt clause in part while its adherence clause survives (Decision 1): outstanding state is tracked because the reminder cannot work without it, but there is no streak, compliance figure, or scoring. 21 of 28 tasks done — the ledger, the scheduler and both UI attempts, switchable in Settings. Notifications are dated one-shots on a rolling horizon, not a repeating trigger (Decision 4), because a repeating trigger cannot be cancelled for one day only. Open: five on-device STOPs, the measurement that sets I and K, and the amendment behind them. |
...
| [Mass Readout](#mass-readout) | ui | 2026-07-26 | In Progress | smol | Display-only mass readout: estimated grams beside carbs wherever scale-truth validation happens. Smolspec reconciled to the post-meal-review architecture (Decision 1): ResultView hero and Records rows shipped with meal-review; the at-capture MealReviewView line implemented 2026-08-10. Remaining: the human-gated device look check. |
| [Glucose Lock Screen Widget](#glucose-lock-screen-widget) | ui | 2026-07-27 | In Progress | full | WidgetKit accessory widget (circular/rectangular/inline + StandBy) showing the most-recent `bsl` reading + a derived trend arrow, fed by an app-written App Group snapshot; status is a non-colour channel that survives the Lock Screen's monochrome rendering, with a dim>15/hide>30 staleness ladder and a `medata://graph` tap target. 24 of 27 tasks done — the device pass (14) closed 2026-08-13 and Decision 16 (accepted) answered the widget going stale while the app is suspended: the extension fetches LibreLinkUp itself under a shared 5-minute vendor budget (cgm-connect Decision 13). Tasks 16.1–16.8 landed and the second device pass (16.7) closed on three hours of tethered logs, settling three things — Decision 17 (the shared gate counts requests sent, so a failing call backs off), Decision 18 (the booked wake is advisory: WidgetKit reloads on its own ~20-minute staleness timer against a 15-minute `staleAge`, so a routine stale window is accepted rather than hidden by raising the threshold), Decision 19 (prime republished an hour-old database reading over the widget's newer one, so the publish needs an ordering guard on `readingDate`). StandBy colour closed in the same pass. Task 16 then closed on 16.9 (the ordering guard now lives in `GlucoseSnapshotStore.write`, so both writers and any future one obey it — Req 1.8) and 16.10 (the measured ~20-minute cadence and its accepted stale window stated as Req 6.5); neither has had a device pass. Remaining: the trend-derivation tightening (15) and the 16.11 device STOP. |
...
| [Build Provenance](#build-provenance) | ui | 2026-08-14 | In Progress | smol | Make the existing build stamp honest rather than add a versioning scheme: `git rev-parse --short HEAD` names the commit, not the tree that was compiled, so a build from a modified working tree claims a clean sha. `BUILD_STAMP` becomes dirty-aware in the Makefile and both deploy scripts, and the standing git-tag convention for UI attempts (`tilt-guide-attempt-N`) is documented rather than replaced. 3 of 5 tasks done; the stub self-report check is in progress and the build/spell/on-device stamp gate remains. |
| [Unified Dark Theme](#unified-dark-theme) | ui | 2026-08-25 | In Progress | full ·iterative | One dark presentation for the whole app via a single `UIUserInterfaceStyle = Dark` plist key, exploiting the grouped tokens resolving to pure black in dark so the OLED capture family and the adaptive grouped family converge without any view changing tokens — the Records → overview → result chain stops flipping palette mid-push. MASTER.md's OLED row goes app-wide, the grouped block is re-headed as structure, the colour block comes level with `Colors.swift`, and `RecordsView` gains the page doc it never had. 4 of 5 tasks done; only the on-device look against the acceptance band remains. |
| [Shared Meal Components](#shared-meal-components) | ui | 2026-08-25 | In Progress | full | The multi-surface features (dose readout, mass line, save-as-quick-add) are the requirement that clears home-router Decision 13's DRY bar: the per-food serving editor (~160 duplicated lines twice), the carb/macro form, the carb-total pair, the corrected marker, the timeline row grammar and the `en_IE` formatters extract into three shared `App/` files, behaviour-preserving through callback seams. Decision 3 records the honest boundary — the shared unit is the content, not the container — and Decision 2 keeps the Graph decoders and corrections-observation loops deliberately duplicated. `MealHistoryModel` dead code is removed. 6 of 8 tasks done; the test gate and the on-device look remain. |
...
| [CGM Direct](#cgm-direct) | data | 2026-08-17 | In Progress | full | Direct-from-device sibling of CGM Connect: a second BLE central rides the worn Libre 3+'s Abbott session and turns each ~1-minute connection/notify event into a reliable background wake for the existing rate-gated LibreLinkUp fetch — bounded ~6-minute worst-case staleness in place of an unbounded OS wake deferral, no glucose value of its own and no new ban exposure (Phase A). Phase B (on-device AES decrypt, zero network) stays research-gated on the living feasibility note. 9 of 16 tasks; stream 2 carries the standing re-verification sweep and the user-controlled uploader routes (region-switch LibreLink, Juggluco bridge). |
| [Fingerprick Glucose](#fingerprick-glucose) | data | 2026-08-21 | Planned | full | Blood readings join the `bsl` stream as the reference measurement: Contour meter samples classified from their Apple Health writer plus a four-interaction hand-entry surface (BSL control on the home Dose row, deep link, launcher widget), held as the reported current value for a tunable 15-minute window without displacing a single sensor row. Each blood reading stamps the sensor's concurrent value and delta, and the trend derives from one provenance at a time. |
| [Manual Carb Intake](#manual-carb-intake) | data · ui | 2026-07-08 | In Progress | full | Manual carb/macro logging without the camera: carb-entry sheet (keypad, 1–999 g, optional macros behind a disclosure), one-tap quick-add presets (`quick_presets` table, seeded defaults), inline edit/delete of manual entries, `EventType.intake` folded into the Graph carb series and Records timeline. Tasks 1–17 done; amended 2026-08-16 with Req 8 — a preset can also be born from a successful capture, so a repeat meal is photographed once and replayed with a tap. Req 8 landed 2026-08-25: schema v10 `source_meal_id` (the one version-gated ALTER), `quickPresetDraft` in MealRouting, and the "Save as quick-add" menu items on both result surfaces; only task 18's on-device checklist remains. |
...
Gives the app a guess at a bolus dose for the meal it has just estimated, computed on-device from a time-banded carbohydrate ratio the developer configures, and records that guess with the inputs that produced it so later ratios and later fat rules can be judged against evidence. The ratio is stored in exactly one canonical direction — `g/U`, grams of carbohydrate covered by one unit of insulin — used in storage, computation, every recorded row and every label; the reciprocal (`5.0 g/U` shown as `= 2.0 U per 10 g`) is derived read-only for display and stored nowhere, so it can never be silently inverted against medreg. Seeded defaults are overnight `10.0`, breakfast `5.0`, lunch `10.0`, dinner `10.0 g/U`, which reproduce the developer's stated rule of 2 U per 10 g in the morning falling to 1 U per 10 g later — a 60 g breakfast at 5.0 g/U suggests 12 U — and they are defaults, not constants, each editable in Settings within a closed `1.0…60.0 g/U` interval. The fat half is deliberately not solved: fat and protein are recorded on every suggestion from the first release and change no number, because a variable uplift folded into the carb term would make every recorded dose `carbs / ratio + unknown` and neither part could be attributed afterwards. Requirement 12 adds the standing basal schedule (nominal 15 U at 07:30 and 19:30) paired with preceding activity but adjusted by nothing. This narrowly reverses the dose-suggestion non-goal recorded in [Regression Suggestion Integration](#regression-suggestion-integration) (Decision 1, annotated in place on that PRD): suggestion arithmetic and insulin-on-board move on-device, parameter **fitting** does not, and `~/repos/medreg` remains the only place parameters are estimated from history. No correction term, no on-device fitting, no confidence display, no automatic recording — a human always taps Save. 14 of 31 tasks in eight phases done — the whole pure calculator (phase 1) and the `dose_suggestions` ledger, plus the two closing housekeeping tasks; phases 6 and 7 are ledgered but unstartable until the evidence each task names exists. The App layer (tasks 9 to 14) was built three times in parallel and none of it is merged: the variants are preserved as `insulin-dosing-ui-attempt-1/2/3`, replayed onto current `research` as `…-on-research-2`, and the pick is a device look, not a code review — task 8 (Decision 15), which blocks everything after it. [design-direction.md](data/insulin-dosing/design-direction.md) owns how the surface looks and reads; [design.md](data/insulin-dosing/design.md) owns the module boundary, the ledger schema and the seam.

- [decision_log.md](data/insulin-dosing/decision_log.md)
- [design-direction.md](data/insulin-dosing/design-direction.md)
- [design.md](data/insulin-dosing/design.md)
- [requirements.md](data/insulin-dosing/requirements.md)
- [tasks.md](data/insulin-dosing/tasks.md)

## Activity Events

Manual activity logging as a first-class event type, so exercise becomes a recorded covariate for insulin dosing long before HealthKit workout data or heart-rate telemetry is wired up. The app records meals, glucose, doses and manual intake in one `events` table and nothing about physical activity, yet the developer's own rule depends on it: the nominal 19:30 basal goes down on active days, more cardiovascular work meaning a larger reduction, with a tail reaching the following morning. `EventType.activity` joins the existing constants under the same timestamp and metadata-JSON conventions, carrying a stable machine kind key — `swim`, `waterpolo`, `cycle`, `run`, `walk`, `gym`, `other` — never the display label, each kind carrying a recorded cardiovascular character of `aerobic`, `anaerobic` or `mixed`. Duration is optional and recorded as absent rather than zero, and every event carries a `manual`/`healthkit` provenance seeded `manual`, so a later HealthKit source is distinguishable without a migration (Decision 4). This spec records the covariate and applies no insulin adjustment of any kind (Decision 5) — the relationship has to be measurable before it can be fitted. Simplicity of capture outranks richness of capture throughout, on the same constraint the dose-entry PRD recorded: if logging is not fast the developer will not do it and the data is worthless. 4 of 9 tasks done: `EventType.activity`, the value types, the store surface (save, delete, the lookback query) and the MedataCore encoding/boundary tests. Remaining: the sheet, the deep link, the graph and records surfaces, and one on-device entry pass.

- [decision_log.md](data/activity-events/decision_log.md)
- [design.md](data/activity-events/design.md)
- [requirements.md](data/activity-events/requirements.md)
- [tasks.md](data/activity-events/tasks.md)

## Dose Schedule

A recurring-dose schedule whose reminder persists until the dose is logged, and whose logging path is short enough that recording a dose costs one tap. The developer takes a standing basal twice a day — nominal 15 U at 07:30 and 19:30 — which [Insulin Dosing](#insulin-dosing) Requirement 12 makes configurable, but nothing prompts for it and nothing records whether it happened. The governing problem is friction rather than motivation: a dose logged through the existing sheet costs unlock, app, Graph, sheet, stepper, kind and save, which is enough work that a twice-daily record will not survive contact with real life, and an intermittent record is worse than none because it biases every fit toward the days logging felt easy. Every quantity is known before the phone is picked up, so the only unknown is whether the dose happened and when, and the input asks for that alone: the one-tap notification action omits `.foreground`, so iOS launches the app in the background, writes an ordinary insulin event, and never shows a screen; a second action opens the pre-seeded sheet for the days activity genuinely changes the amount. The repeat mechanism stays free of background execution — the due notification plus K follow-ups are scheduled up front and the remainder cancelled on discharge — so nothing competes with the CGM `BGAppRefreshTask` budget. This reverses insulin-dosing Req 12.7 in part, and only in part: the no-prompt clause goes, the adherence clause survives (Decision 1) — outstanding state is tracked because the reminder cannot work without it, but there is no streak, no compliance figure, no scoring, no reason asked for a skip, and no computed amount. 21 of 28 tasks done; the six device STOPs and the amendment behind them remain.

- [decision_log.md](data/dose-schedule/decision_log.md)
- [design.md](data/dose-schedule/design.md)
- [requirements.md](data/dose-schedule/requirements.md)
- [tasks.md](data/dose-schedule/tasks.md)
...
A WidgetKit accessory widget added as a new data-driven kind inside the existing `MeDataWidgets` extension, surfacing the current/most-recent blood-glucose reading and a trend arrow on the Lock Screen and StandBy with a glance and no interaction beyond a tap to the Graph. The app publishes a versioned, atomically-written App Group snapshot (mmol/L value, timestamp, derived trend, target-band status) on `eventsDidChange` and reloads only the glucose kind; the widget renders from that snapshot and, since Decision 16, refetches it (no GRDB in the extension). Trend is a pure `GlucoseTrendMath` function (least-squares slope over the last 30 min, guarded by a ≥10-min span), status is a non-colour glyph/token that survives the Lock Screen's monochrome vibrant rendering (colour only as a StandBy-day enhancement), and staleness is a pure render-point function (full ≤15 min → dimmed >15–30 → "last reading · Xh ago" >30, distinct from never-recorded) kept Foundation-only, with the WidgetKit `TimelineEntry`/reload-policy adaptation confined to the extension (Decision 12). Reuses the `bsl` stream from cgm-connect/libre-ingestion; no estimation-path code. 24 of 27 tasks done across the shared contract + pure logic, app integration, and widget-extension streams. The device pass (task 14) closed 2026-08-13 — renders and App Group round-trip fine — but surfaced the widget going stale whenever the app is suspended; Decision 16 (accepted) answers it: the extension fetches LibreLinkUp itself in `getTimeline` under the shared 5-minute vendor budget (cgm-connect Decision 13), auth app-owned, database still the source of record — implementation ledgered as tasks 16.1–16.8, all landed (LibreLinkUpKit extracted, derivation shared, uniform 5-minute baseline, shared rate gate, shared-state migration, extension-side fetch, and the Decision 17 wake backoff in which the shared gate records requests *sent* rather than successful fetches, so a failing vendor call costs one request per interval instead of as many as WidgetKit will run). The second device pass (16.7) closed 2026-08-13 on three hours of tethered logs and settled two open questions. Scheduling: the booked `.after(date)` is advisory, not a schedule — 14 of 18 reloads in the window fired as `reason: stale` and none from the booked date, so WidgetKit's own staleness timer drives the cadence at roughly 20 minutes against a 15-minute `staleAge`. Decision 18 accepts the resulting stale window rather than closing it, because `staleAge` is a truth threshold and also the reload trigger: raising it would postpone the refresh by the same interval it bought while relabelling stale data as fresh. The same logs caught prime republishing an hour-old database reading over the newer one the widget had fetched while the app was suspended, self-corrected 274 ms later by the next tick — Decision 19 makes the publish monotonic in `readingDate` — landed as task 16.9 with the ordering guard inside `GlucoseSnapshotStore.write` rather than the publisher, so the extension's own writes and any future third writer are covered too (Req 1.8), while a never-recorded snapshot still clears the tile; 16.10 states the measured cadence and its accepted stale window as Req 6.5. Neither has been device-verified. StandBy colour closed in the same pass: high renders orange, in-range is `.primary` by design (an absent token *is* the in-range state), and low is accepted on construction, sharing the verified branch's `renderingMode` guard and differing only in the colour literal. The trend-derivation tightening against observed CGM cadence (task 15) also remains, now more valuable with a true 5-minute feed.
...
The ask was to add commit tags or hashes for UI version changes; surveying what exists, most of it is already met and the rest fights a standing convention. A commit hash is already stamped into every Make-built binary (`BUILD_STAMP`, injected as `MEDATA_BUILD_STAMP`, substituted into the `MedataBuildStamp` Info.plist key, logged at launch as `event=launch buildStamp=… segmenterSource=…`, and written onto every correction record); git tags for UI attempts are already the practice (`tilt-guide-attempt-1`, `tilt-guide-attempt-2`); numbered design handoffs already version the design source. So the spec narrows to the one real gap, and it is a correctness gap rather than a versioning gap: the stamp is not dirty-aware. `git rev-parse --short HEAD` names the commit, not the tree that was compiled, so a build made from a modified working tree — the normal state during UI iteration — claims the clean HEAD sha, and two builds from different trees at the same commit are distinguished only by a wall-clock suffix. 3 of 5 tasks done: the Makefile stamp is dirty-aware and both deploy scripts mirror the same default, and the stamp format plus the UI-attempt tag convention are documented. Remaining: the stub self-report check (in progress) and the build/spell/on-device stamp gate.

- [decision_log.md](ui/build-provenance/decision_log.md)
- [smolspec.md](ui/build-provenance/smolspec.md)
- [tasks.md](ui/build-provenance/tasks.md)

## Unified Dark Theme

One dark presentation for the whole app, delivered as a single `UIUserInterfaceStyle = Dark` key in `MeData/Info.plist`. The mechanism exploits a property the token set already has: under dark resolution `.systemGroupedBackground` (`surfacePrimary`) is `#000000` — identical to `captureBackground` — and `.secondarySystemGroupedBackground` is `#1C1C1E`, so the OLED capture family and the adaptive grouped family converge rather than needing to be merged, and the grouped family survives as structure (base vs elevated card) rather than a light/dark split. No view changes tokens; the adaptive definitions stay so a future light theme is a one-line revert. The written design system reconciles in the same pass: MASTER.md's "Dark Mode (OLED)" row goes app-wide with its original scope preserved in the annotation, the colour block comes level with `App/Colors.swift`, the meal-overview and data page docs carry in-place amendment notes, and `design-system/pages/records.md` is authored for the surface that never had a page doc. 4 of 5 tasks done; the closing gate is the on-device look against the acceptance band (every route dark, no palette flip, capture flow byte-identical).

- [decision_log.md](ui/unified-dark-theme/decision_log.md)
- [design.md](ui/unified-dark-theme/design.md)
- [requirements.md](ui/unified-dark-theme/requirements.md)
- [tasks.md](ui/unified-dark-theme/tasks.md)

## Shared Meal Components

The extraction the repo twice declined for its own sake, now demanded by a requirement: the dose readout (insulin-dosing Req 6.1/6.10), the mass line (mass-readout) and save-as-quick-add (manual-carb-intake Req 8.1) each land on three-plus surfaces, which is exactly the "forced coordination" home-router Decision 13's bar asks for. Three shared `App/` files — `ServingRows.swift` (PlateFraction, amount button, gram editor, step buttons and arithmetic, converged on 44 pt targets), `MealReadouts.swift` (`CarbAmountText`, one `CorrectedMarker` for its four sites, `RecordedSuggestion.line`, the relocated `MealPhotoLoader`) and `SharedFormatting.swift` (cached `en_IE` formatters, `prettify`, the `TimelineRow` grammar behind the five Records rows and the Intake entry row) — plus `CarbAmountField`/`MacroDisclosure` shared between the entry sheet and the preset editor. Behaviour-preserving through callback seams that keep each surface's persistence in place; accessibility identifiers byte-identical. Decision 2 keeps the Graph decoders and the corrections-observation loops deliberately duplicated; Decision 3 records that the shared unit is the content, not the container. `MealHistoryModel` is removed, its live types rehomed. 6 of 8 tasks done; the test gate and the on-device look remain.

- [decision_log.md](ui/shared-meal-components/decision_log.md)
- [design.md](ui/shared-meal-components/design.md)
- [requirements.md](ui/shared-meal-components/requirements.md)
- [tasks.md](ui/shared-meal-components/tasks.md)
...
Direct-from-device sibling of [CGM Connect](#cgm-connect). Phase A connects a second CoreBluetooth central to the worn Libre 3+ alongside Abbott's app — never pairing, never decrypting — and uses each ~1-minute notify event as a background wake that triggers the existing rate-gated LibreLinkUp fetch after a 1 s let-Abbott-upload delay. The honest gain is a bounded ~6-minute worst-case staleness (gate interval + one beat) replacing the deferred-BGAppRefreshTask failure of 2026-08-05, within the one shared vendor budget. Pairing is confirm-once/retrieve-thereafter (background wildcard scanning does not exist on iOS; a sensor swap is a foreground re-pair, Decision 8); OS-driven background relaunches skip the gate-ignoring validation fetch (Decision 7); a regional App Store restriction on Abbott's LibreLink is routed around with documented user-controlled steps — region-switch to the sensor's country, or the Juggluco bridge — never accepted as a block (Req 10, Decision 11, `docs/libre-app-region-setup.md`). Phase B — on-device AES-128-CCM decrypt under a distinct `libre3-ble` source id — remains gated on `docs/agent-notes/libre3-direct-ble.md`, which carries a dated re-check log (Req 8).

- [decision_log.md](data/cgm-direct/decision_log.md)
- [design.md](data/cgm-direct/design.md)
- [prerequisites.md](data/cgm-direct/prerequisites.md)
- [requirements.md](data/cgm-direct/requirements.md)
- [tasks.md](data/cgm-direct/tasks.md)

## Fingerprick Glucose

Blood glucose measurements as first-class `bsl` events carrying blood provenance, arriving through Apple Health from a Contour Next meter (per-writer classification in Settings, no hard-coded bundle id) or a hand-entry surface reachable in at most four interactions from the new BSL control on the home Dose row, a `medata://glucose/add` deep link, and a fourth launcher widget. A blood reading is the reference measurement: stored off the 5-minute grid at its exact instant, stamped with the sensor's concurrent value and signed delta (the raw material for later sensor-error analysis), and reported as the current value on every surface for a tunable hold window (default 15 minutes) without modifying, hiding, or deleting any sensor row. The trend derives from one provenance at a time — sensor first, blood alone when no sensor series qualifies — and the latest-reading display on home becomes a route to Graph, superseding home-router Req 4.8. Extends the snapshot store's monotonic guard with a hold-aware merge policy and a deletion-authorised rollback. 23 tasks, none started; the meter route needs the Contour Diabetes app syncing to Apple Health (prerequisites.md).

- [decision_log.md](data/fingerprick-glucose/decision_log.md)
- [design.md](data/fingerprick-glucose/design.md)
- [prerequisites.md](data/fingerprick-glucose/prerequisites.md)
- [requirements.md](data/fingerprick-glucose/requirements.md)
- [tasks.md](data/fingerprick-glucose/tasks.md)

## Manual Carb Intake

A direct manual carb-logging path alongside the photo pipeline: a carb-entry sheet modelled on the insulin-dose flow (numeric keypad, whole grams 1–999, back-dateable timestamp, optional protein/fat/fibre behind a disclosure — absent, never zero, when left empty) and one-tap quick-add presets (flat user-editable collection in the new `quick_presets` table, schema v5, seeded once with "A pint"/"Bagel"/"Chips"). Entries are `EventType.intake` events mirroring the insulin convention, distinguishable from photo meals, folded into the same Graph carb series and Records timeline, and editable/deletable inline from the Intake surface's recent-entries list. A manual entry can be saved as a new preset in one step. Tasks 1–11 done — the on-device verification checklist (design.md, Testing Strategy) awaits the human loop.

Amended 2026-08-16 (Req 8, Decisions 7–12): a quick-add preset gains a second origin — a successful capture. "Save as quick-add" sits in the overflow menu of both result surfaces (post-capture review and the result reached from Records) and pre-fills the existing preset sheet with the meal's *displayed, correction-adjusted* carbohydrate total and a name derived from its detected foods. The per-class decomposition is dropped, not stored: replay is the ordinary one-tap quick-add writing an `intake` event, never a reconstructed meal. `quick_presets` gains a nullable `source_meal_id` (schema v9, the store's first additive `ALTER TABLE`) as a point-in-time stamp that is written and never dereferenced, mirroring the entry-side `preset_id`. Freezing a β = 1 over-read is accepted deliberately, mitigated by the value being editable before it is saved (Decision 11). Tasks 12–18 open.
