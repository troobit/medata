...
| [Support Plane Reference](#support-plane-reference) | estimation | 2026-07-27 | In Progress | full | Correct the LiDAR support plane from the table (measured 26.1 mm too low, a 2–3.6× volume over-read) to the surface the food actually rests on: an annulus-bounded, native-depth-grid CC-RANSAC restricted fit with radial-band + angular-sector ring guards, falling back to the existing edge-band fit — with a documented, corpus-measured fallback rate — when the fit can't be trusted. 8/27 tasks done (the geometry-and-selection phase: depth-intrinsics derivation, contact-ring/ring-statistics, bounded CC-RANSAC extraction, and `fitFoodSupportPlane`'s admissibility guards and selection); wiring/persistence, harness parity, fixtures and the human-gated device verification remain. |
| [Alternative Class Candidates](#alternative-class-candidates) | estimation | 2026-08-07 | No Tasks | full | Retain the per-detected-food alternative classes the segmenter computes and discards, so a relabel shortlist can offer a food the user has never chosen. Owed by meal-review Decision 18, which deferred it and shipped recency ordering. Recency is kept as a live component of the ordering rather than superseded (Decision 6), so a record without candidate evidence yields the shortlist meal-review would have produced and no correction path can regress. At its requirements gate — requirements + decision log written, design not started. |
...
| [Home Router](#home-router) | ui | 2026-07-07 | In Progress | full | Home page becomes the launch root (no tab bar): six routed full-screen covers with Capture primary, unified Records timeline (meals + insulin + glucose, delete for manual entries only — since superseded by Records Deletion), Graph demoted to visualisation-only. 9/11 tasks done; the on-device visual verify (task 9) and the latest-glucose header STOP (task 11) are human-gated. |
...
| [Glucose Lock Screen Widget](#glucose-lock-screen-widget) | ui | 2026-07-27 | In Progress | full | WidgetKit accessory widget (circular/rectangular/inline + StandBy) showing the most-recent `bsl` reading + a derived trend arrow, fed by an app-written App Group snapshot; status is a non-colour channel that survives the Lock Screen's monochrome rendering, with a dim>15/hide>30 staleness ladder and a `medata://graph` tap target. 13 of 15 tasks done — the MedataCore contract and pure maths, the app-side publisher and `medata://graph` route, the App Group + extension wiring, the widget kind and its per-family views, the agent notes, and the build/test/spell verification sweep are in; the human-gated device pass and a follow-up tightening of the trend derivation against the observed CGM cadence remain. |
| [Meal Review](#meal-review) | ui | 2026-08-06 | Planned | full | Collapse the read-only segmentation review and the result screen into one post-capture surface: detected foods outlined on the photo, relabel and reject per food, serving/gram amounts, and one tap to record. The deliverable is the corpus — one correction record per detected food per capture, predicted values beside corrected ones, never discarded on age, count, or any sweep. Requirements, design and prerequisites approved; 22-task ledger written, implementation not started. |
...
| [CGM Connect](#cgm-connect) | data | 2026-07-10 | In Progress | full | Live glucose ingestion behind a source abstraction (HealthKit primary, LibreLinkUp follower complement) writing `bsl` events with cross-source 5-minute-grid dedup, firewalled from estimation by a package-graph test. 17/19 tasks done; the on-device verify (task 14) and the adaptive-poll-interval STOP (task 16) are human-gated. |
...
The LiDAR support plane fits the table rather than the surface the food rests on — measured 26.1 mm too low, producing a 2–3.6× volume over-read that is proportionally worst on the flattest food (two weighed captures: 2 slices of bread read 3.57× high, 208 g of rice read 2.1× high). Replaces "largest gravity-aligned plane in the frame" with "the gravity-aligned plane the food is resting on": candidate samples are bounded to an annulus (2×`ringOuterMm`) around the food mask on the native depth grid (Req 2.4's ~56× replication removed), scored by CC-RANSAC's largest 8-connected inlier component rather than raw inlier count, and selected on a contact ring resolved into radial bands (well/rim/bowl/leaked-to-table) plus angular sectors — the sector guard is what stops a ring 65% on the table from being certified as a correct fit while reading a misleadingly healthy aggregate support fraction and a near-zero median (Decision 18, the design's most dangerous identified failure mode). Rejection routes to the existing edge-band fit unchanged, with the fallback rate reported per corpus rather than assumed low. The geometry-and-selection phase (tasks 1–8) is in: `SupportRegion.depthIntrinsics`/mask downsampling, `contactRing`/`ringStatistics` (per-band medians, inner-band support fraction, angular sector support, support visibility), bounded CC-RANSAC extraction with adaptive per-pass iteration stopping, and `fitFoodSupportPlane`'s admissibility guards (signed median band, inner→mid band step, food upper-envelope, annulus-escape, extent) plus ambiguity-margin selection. Remaining: wiring the restricted fit ahead of the lazy edge-band fallback and extending `EstimationAttemptRecord`/calibration-artefact persistence (tasks 9–14), harness parity and fallback-rate reporting (15–21), regression fixtures and cleanup of the superseded flood-fill path (22–24), document amendments (25), the corpus measurements the design deliberately left as placeholders (26), and the human-gated on-device weighed-food verification (27).
...
The segmenter computes a probability over every palette channel for every pixel, takes the argmax, and discards the rest — so on every capture the model states what else each region might have been, and the app throws it away. `perClassMeanProb` cannot stand in: it is accumulated only where a class won the argmax, so it holds an entry only for classes already on the plate, and its value is the winner's own mean confidence. This spec retains the discarded evidence per detected food and persists it on the meal record, to be consumed by the relabel shortlist in meal-review Req 3.1. Recency is explicitly not the thing being replaced (Decision 6): for a single-user app whose owner eats the same foods repeatedly it is a strong prior, and it stays a live component of a combined ordering — a food carrying recency is never demoted for lacking candidate evidence, and a record without evidence yields exactly the shortlist meal-review would have produced. Candidates are computed over the regularised label map rather than in the existing σ_seg loop, which runs pre-regularisation by documented contract (Decision 3), at a stated latency budget on the hardware floor with an abandonment escape. The acceptance bar is a measured shortlist hit-rate gain over recency alone, split by first-versus-repeat correction; a neutral result is a valid outcome. Owed by meal-review Decision 18 and filed 2026-08-07 at its requirements gate — design not yet written, so no task ledger exists.
