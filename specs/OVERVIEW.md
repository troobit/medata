...
| [Insulin Dosing](#insulin-dosing) | data | 2026-08-14 | Planned | full | An on-device guess at the bolus dose for a meal just estimated, from a time-banded carbohydrate ratio stored in exactly one canonical direction — `g/U`, grams of carbohydrate covered by one unit — with the reciprocal rendered read-only beside it and never stored. Seeds overnight 10.0, breakfast 5.0, lunch 10.0, dinner 10.0 g/U, reproducing the developer's rule of 2 U per 10 g in the morning falling to 1 U per 10 g later. Fat and protein are recorded on every suggestion and change no number. Narrowly reverses [Regression Suggestion Integration](#regression-suggestion-integration)'s dose-suggestion non-goal (Decision 1): suggestion arithmetic and insulin-on-board move on-device, parameter fitting stays in `~/repos/medreg`. 25 tasks, none started. |
| [Activity Events](#activity-events) | data | 2026-08-14 | Planned | full | `EventType.activity` as a first-class manual event so exercise becomes a recorded covariate long before HealthKit workouts are wired up: a stable machine kind key (swim, waterpolo, cycle, run, walk, gym, other) each carrying a fixed aerobic/anaerobic/mixed character, optional duration recorded as absent rather than zero, and `manual`/`healthkit` provenance so a later source needs no migration. Records the covariate and adjusts no dose (Decision 5). 9 tasks, none started. |
