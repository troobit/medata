...
- The follow-on full spec `specs/estimation/pipeline-real-device-correctness/` (now landed) is the more attractive long-term measure: a real food-region mask from segmentation shrinks the candidate band substantially. That's still a perf-not-correctness win; the algorithmic fix here is the durable one.
