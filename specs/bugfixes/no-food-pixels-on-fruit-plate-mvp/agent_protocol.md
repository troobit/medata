...
1. Build the App scheme for the device destination (`xcodebuild ... -destination 'id=<device-udid>'`) and confirm `** BUILD SUCCEEDED **`.
2. Print the `xcrun devicectl device install app --device <device-udid>` and `xcrun devicectl device process launch ... rtob.MeData` commands the user runs.
