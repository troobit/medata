...
Tasks prefixed with **STOP —** are human-in-loop device-verification phases. The Claude agent driving this spec CANNOT build for the iPhone, install via `devicectl`, or read live Console logs without the user. When a STOP task is reached, the agent MUST:
...
- [ ] 4. Add cadence-diagnostic `.info` instrumentation on a throwaway branch <!-- id:qx7n2ta -->
  - In `MedataCore/Sources/CaptureKit/ARKitCaptureEngine.swift`:
    - Inside the `frames` getter, log `os_log .info` immediately after `frameContinuations[id] = cont` with the id and `count=frameContinuations.count` so we can see the registration ordering.
    - Inside `session(_:didUpdate:)`'s `stateQueue.async` block, log `os_log .info` per yield with the continuation count (`yielded count=N`).
  - In `App/PreShutterSegmenter.swift`'s `resume(frames:)` for-await body:
    - Log `os_log .info` on each for-await iteration entry (`event=preshutter.loop.iter`).
    - Log `os_log .info` on entry to `makeRawFrame` and on the post-convert guard branches (success vs nil).
    - Log `os_log .info` after `segment(_:)` returns (success / nil).
    - Log `os_log .info` after `publish()` returns (`event=preshutter.loop.published`).
  ...
  - Blocked-by: op0jbkx (swift build clean and swift test green)
...
- [ ] 7. swift build clean and swift test green (no regressions) <!-- id:qx7n2td -->
  ...
  - Blocked-by: qx7n2tc (Apply minimal cadence fix; remove instrumentation; append ## Root cause + ## Fix to smolspec.md)
...
- [ ] 8. **STOP — human verifies on device in Single AND Double modes; agent diagnoses success or new failure** <!-- id:op0jbky -->
  ...
  - Success criteria (BOTH modes):
    - `event=estimate.start maskAgeMs=N` with `N >= 0` AND `N <= 750`.
    - `event=estimate.end success=true mealId=… capturePath=…`.
    - ResultView renders (carbs + confidence visible). User confirms.
...
- [ ] 9. Append `## Verification` section to smolspec.md with build SHA and observed success trail (both modes) <!-- id:op0jbkz -->
  ...
  - Blocked-by: op0jbky (STOP — human verifies on device in Single AND Double modes)
...
- [ ] 10. Append `### On-device observation (complete, rerun)` block to shutter-blocked-feedback/decision_log.md; tick task 5 (id:f4inr0r) in shutter-blocked-feedback/tasks.md <!-- id:op0jbl0 -->
  ...
  - Blocked-by: op0jbkz (Append ## Verification section to smolspec.md)
