...
#### Known divergence: video-range YCbCr is decoded with the full-range matrix (Decision 8)

The supported-source set above is *full-range* biplanar YCbCr (`kCVPixelFormatType_420YpCbCr8BiPlanarFullRange`), BGRA8, and RGBA8 — [Req 1.1](requirements.md#1.1) scopes only full-range, and [Req 1.5](requirements.md#1.5) mandates that any other format throws `unsupportedSourceFormat`. The shipped implementation (`PixelBufferAdapter.swift`) additionally accepts `kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange` and routes it through the **same full-range** `vImage_YpCbCrToARGB` info struct. This produces a correctly-sized, correctly-ordered BGRA8 buffer, so the length/format contract and the pre-processor's `validateInputSize` both still pass — but the *colour* contract silently breaks: a video-range buffer (Yp 16..235, CbCr 16..240) decoded with full-range coefficients yields contrast-stretched, slightly off-hue pixels that `canonicaliseToRGB8` cannot detect. ARKit's `ARFrame.capturedImage` is full-range by default, so on the current single-mode capture path this branch is dormant; it becomes a real defect only if a device/ARSession configuration ever delivers video-range frames. This is the residual "pre-processor mismatch on real device captures" risk. Decision 8 records the divergence and the required reconciliation (decode video-range with a video-range info struct, or honour Req 1.5 and throw).
...
| Test | What it covers |
|---|---|
| `testFlatGrayYCbCrProducesGrayBGRA` | Constant Y=128, Cb=Cr=128 → every output BGRA pixel ≈ (128,128,128,255) within ±2. Covers [1.1](requirements.md#1.1), [1.2](requirements.md#1.2), [1.4](requirements.md#1.4). |
| `testKnownColourPatchMatchesBT601` | 2×2 patch with Y/Cb/Cr values for red, green, blue, white per BT.601 full-range formulas; output within ±2. Covers [1.2](requirements.md#1.2). |
| `testOutputIsContiguousNoStridePadding` | Build a 6×4 buffer (width × 4 = 24 bytes/row, not stride-aligned), assert `bytes.count == 6*4*4` and that decoded row N starts at offset `N*24`. Covers [1.3](requirements.md#1.3). |
| `testBGRASourcePassesThrough` | Source is `kCVPixelFormatType_32BGRA` with known bytes; output bytes == input bytes; `format == .bgra8`. Covers [2.1](requirements.md#2.1), [5.2](requirements.md#5.2). |
| `testRGBASourceReportsRGBA` | Source is `kCVPixelFormatType_32RGBA`; `format == .rgba8`; bytes round-tripped. Covers [2.2](requirements.md#2.2). |
| `testYCbCrSourceReportsBGRAOnOutput` | Source is biplanar YCbCr full-range; `format == .bgra8` (describes output). Covers [2.3](requirements.md#2.3). |
| `testUnsupportedFourCCThrows` | Source is `kCVPixelFormatType_422YpCbCr8` (a format the adapter does not handle); `ConversionError.unsupportedSourceFormat(fourCC: "2vuy")` is thrown. Covers [1.5](requirements.md#1.5), [2.4](requirements.md#2.4). |
| `testNonSquareDimensions` | 640×480 YCbCr buffer with row-major colour ramp; verify each row's first pixel matches the formula. Covers [1.1](requirements.md#1.1), [1.3](requirements.md#1.3). |
| `testRandomisedRoundTripWithinTolerance` (PBT-style) | Use the deterministic seeded RNG pattern from `CardPosePropertyTests` to generate 200 random (Y, Cb, Cr) triples, build a 1×1 buffer per triple, assert each output channel is within ±2 of the BT.601 reference. Covers the universal-guarantee aspect of [1.2](requirements.md#1.2). |
...
Builds a synthetic YCbCr `CVPixelBuffer`, runs it through `PixelBufferAdapter.convert`, hands the resulting `(bytes, format, width, height)` directly to `SegmenterPreProcessor.process(...)` and asserts no `SegmentationError.invalidInputDimensions` is raised. Covers [5.1](requirements.md#5.1). The test depends on both targets via the existing `dependencies: ["Segmentation", "CaptureKit", "PortableContracts"]` in `Package.swift:134`.

### App-layer catch-all rewrite — `MeData/Tests/CaptureFlowModelInternalErrorTests.swift` (new or extend)
...
- **App-layer test seam may not exist.** *(Resolved as implemented.)* `CaptureFlowModel` stores its pipeline behind a `PipelineEstimator` protocol seam (`estimate(captureResult:)`), so the synthetic-error test in `MeData/Tests/CaptureFlowModelInternalErrorTests.swift` injects a throwing `ProgrammablePipeline` fake without an additional refactor. The synthetic `TestErrorType` drives the catch-all and the test asserts `state == .refused(.internalError("TestErrorType"), …)` and explicitly `!= .noScaleAvailable`, covering [Req 6.5](requirements.md#6.5).
