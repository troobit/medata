...
3. <a name="3.2a"></a>WHEN the most recent fetched reading is below the urgent threshold, OR the fetched readings are falling at or faster than the urgent fall rate, THEN the system SHALL shorten the next fetch interval to the urgent interval; otherwise it SHALL use the baseline interval. The urgent interval SHALL NOT be shorter than 5 minutes, the rate at which the vendor is known to ban accounts being ~3 minutes (Decision 12).
