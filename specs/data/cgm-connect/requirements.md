...
2. <a name="3.2"></a>WHILE the LibreLinkUp source is connected, the system SHALL fetch new readings at a 5-minute interval while the app is foregrounded, and SHALL schedule a best-effort background fetch; WHEN the app is next opened, the system SHALL perform a catch-up fetch (Decision 13).
3. <a name="3.2a"></a>The fetch interval SHALL NOT be shorter than 5 minutes, the rate at which the vendor is known to ban accounts being ~3 minutes. WHEN vendor rate-limiting or ban signals are observed, the baseline SHALL be rolled back to 15 minutes with Decision 12's adaptive tightening re-armed — that machinery is retained dormant in code as the designated rollback position (Decision 13).
...
1. <a name="5.1"></a>The system SHALL snap each incoming reading's instant to the nearest 5-minute grid mark and SHALL treat a reading as already present WHEN a stored `bsl` event carrying sensor provenance exists at that grid instant, regardless of source, appending only readings at grid instants not yet covered — extending the keep-first rule of `specs/data/libre-ingestion` [Req 5.2](../libre-ingestion/requirements.md#5.2) across sources onto one shared grid. *(Narrowed to sensor provenance by `specs/data/fingerprick-glucose`: blood-provenance events are stored off-grid at their exact instants and SHALL NOT count as occupying a grid mark — [Req 1.3](../fingerprick-glucose/requirements.md#1.3), [Req 7.2](../fingerprick-glucose/requirements.md#7.2). Superseded wording: "WHEN a stored `bsl` event exists at that grid instant".)*
