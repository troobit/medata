...
- Measuring or reporting sensor-versus-blood error; this feature preserves the paired data so it can be measured later, and analyses none of it.
...
1. <a name="2.1"></a>The system SHALL present a glucose-entry surface WHEN the latest-reading display on the home page is tapped, and WHEN the `medata://glucose/add` deep link is opened, from any app state.  
...
**User Story:** As a user, I want the trend arrow to keep meaning what it meant, so that one blood reading does not make a steady sensor trace look like a spike.

**Acceptance Criteria:**

1. <a name="5.1"></a>The system SHALL derive the trend indicator from sensor readings alone, excluding blood readings from the derivation, WHILE the reported current value may itself be a blood reading.  
2. <a name="5.2"></a>WHERE the available sensor readings do not satisfy the existing span requirement for a trend, the system SHALL report no trend rather than deriving one from blood readings.  
