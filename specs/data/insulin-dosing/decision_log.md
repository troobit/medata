...
The suggested meal bolus is carbohydrate grams ÷ band ratio, less only the **unoffset** insulin-on-board — boluses with no meal or intake behind them (correction or freestanding doses); a bolus associated with a recorded meal or intake (the Req 7.5 association or its 45-minute window) never reduces a later meal's coverage (new Req 4.8). Results are whole units only (the 0.5 U increment is removed), and there are no suppressed outcomes: a meal with a carbohydrate total always shows its number, `0 U` included. Tapping the readout opens the working — `carbs ÷ ratio = y U`, each reduction as `− x U, for <reason>`, result — on every surface, reconstructed verbatim from the recorded row on history surfaces (Req 6.12). The readout is **the meal's quality pill**: the capsule the confidence tier used to hold, at that view's exact metrics, containing the `syringe` glyph and the whole-unit figure, filled with the confidence tier's own colour. Confidence stops being rendered as its own named pill on the capture surfaces and becomes that fill, named in words in the working under the calculation; the raw σ remains recorded per capture on the meal record. The dose is no larger than the carbohydrate total, never takes the `Record` slot, and writes nothing — that last one is the invariant that keeps it from being an instruction, and it is the only one that ever was. In the ledger, the insulin-on-board field keeps recording the physiological total (the field means what its name says); the subtraction the estimate applied is recorded as its own reduction field, capped at the base value so the recorded working sums exactly — a derived term is never recorded under its source's name (Req 7.2). The history surface is the single meal-detail screen of `specs/ui/home-router` Decision 16 (overview page and DEBUG `⋯ → Review` push deleted), and the demo-meal seeder writes its `dose_suggestions` row at seed time so a seeded meal shows its figure with zero navigation.
...
The readout's presentation is settled 2026-08-28 across three passes on device against a seeded meal (the arithmetic above was settled 2026-08-26 and is unchanged). Every prediction this spec made on paper about visual weight failed, and all of them failed the same way.

Pass 1 held that a glyph beside the dose would read as a call to action. It reads as a type label, and its absence was the defect: a measured mass and the dose derived from it rendered identically, so `≈ 380 g on plate · 6 U` read as two measurements of one meal. Pass 2 held that any tap affordance was control chrome; with none, the working was undiscoverable, and a provenance route nobody can find is not a provenance route. Pass 3 is this one: a glyph and an underline still left the dose quieter than the model's own self-assessment sitting in the pill beside it.

That last observation is what forced the inversion rather than another tweak. A confidence tier is something a person reads off their own plate faster than the model computes it; the dose is the number they cannot derive by looking. Spending the surface's most prominent chrome on the former and `subheadline` grey on the latter had the priority backwards. Making the dose the pill and the confidence its fill puts the accuracy signal on the thing it qualifies, and costs nothing in layout because the two views share geometry exactly.

The general lesson, recorded because it cost three device passes: judgements about how a screen *will* be read are not evidence, and this spec made three of them in a row in the same wrong direction. Each was defensible on paper and each over-weighted the risk of the dose looking like a command against the certainty of it being missed. `rule_version` bumps so rows computed under the superseded rule stay separable (Req 7.9).
...
- The readout is filled, tappable and looks like a control, which is what design-direction §2.2 spent three revisions avoiding. "Never a control" is now only "never writes". A reader who takes a filled capsule as something that acts will take this one that way, and the sole thing standing against that reading is that tapping it opens a sheet and changes nothing.
- At High confidence the pill's fill is the app accent, so a high-confidence dose is the same green as `Record`. Inherited from the confidence pill, which always rendered High that way on this screen, but the collision now lands on the dose rather than beside it.
- Confidence loses its own named pill on the capture surfaces. A reader who wants the tier in words must tap through to the working; the colour alone carries it at a glance, and colour-blind readers get the tier from the working and the recorded σ rather than from the fill.
...
`MedataCore/Sources/Dosing/DoseSuggester.swift` (unoffset insulin-on-board input, no suppression outcomes, `ruleVersion` bump), `App/DoseSuggestionModel.swift` (offset-aware insulin-on-board query; increment fixed at 1 U), `App/ConfidencePill.swift` (new `DosePill`; `ConfidencePill` retained for surfaces reporting confidence as the subject), `App/MealReviewView.swift` + `App/ResultView.swift` (pill swap, sigma into the working sheet), `App/DoseReadoutLine.swift` (`MealTotalSecondLine` reduced to plate mass), `App/MealReadouts.swift` (`DoseHistoryLine` reduced to the `given` figure), `App/DoseWorkingSheet.swift` (confidence legend under the calculation), ratio run removed per design-direction §10, the Settings increment row (removed), `App/SettingsView.swift`'s demo-meal seeder (writes the suggestion row), and the routing deletions of `specs/ui/home-router` Decision 16. Requirements 3.1, 3.4, 4.7, 5.1, 5.6, 5.7, 6.6, 6.10 amended and 4.8, 6.12 added above.
