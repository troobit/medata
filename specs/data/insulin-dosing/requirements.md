...
10. <a name="6.10"></a>WHERE a recorded suggestion (Requirement 7) exists for a meal or intake, the meal detail surfaces reached from history — the meal overview and the full result screen — SHALL render it as a zero-tap read-only value alongside the carbohydrate total, in the same derived register and middle-dot grammar as the review surface, showing the rounded suggested units and, WHERE a dose has been associated (Req 7.5), the given units beside them.
