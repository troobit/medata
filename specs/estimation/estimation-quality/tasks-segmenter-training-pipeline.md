...
- [x] 10. STOP — R3: attribution follow-up on R1, shape set by R1's verdict <!-- id:pgctxef -->
  ...
  - NOT swapped: the bundled segmenter.mlpackage stays ab812dc3aa9d. Export, the gates and an on-device capture pass are the separate gated step — SCHEDULED 2026-09-10 as task 11 (id r3swap1), which carries the commands
...
- [ ] 11. STOP — export R3 to Core ML and swap the bundled segmenter.mlpackage (M5 Pro Mac, then iPhone 16 Pro) <!-- id:r3swap1 -->
  - THE HIGHEST-VALUE OPEN MVP ACTION, and until now it existed only as a sentence inside task 10 saying it was "unscheduled". R3 (`0a019c00e943`) is the adopted recipe and reference checkpoint (segmenter-foundation Decision 36) and beats the shipped incumbent `ab812dc3aa9d` by +0.0265 mean food-class IoU on the 182-image leak-free anchor — and none of it reaches the phone, because the bundle still carries the incumbent. Every remaining on-device STOP on the capture-to-estimate path sits behind this one
  - NOT agent-executable from the Intel machine: no Xcode (26.6 requires Apple M1 or later), no MPS, and no checkpoints or datasets present locally. Runs on the M5 Pro Mac at `/Users/r/repos/medata`, then the phone
  - Step 1 — sync: `cd /Users/r/repos/medata && git checkout research && git pull origin research`
  - Step 2 — export ASIDE, never straight into `Resources/` (docs/ml-training.md section 7): `tools/segmenter/.venv/bin/python tools/segmenter/export.py --checkpoint tools/segmenter/build/checkpoint_r3_combined_noweight.pt --out-coreml tools/segmenter/build/segmenter-0a019c00e943.mlpackage --lineage /Users/r/repos/medata/tools/segmenter/build/lineage-0a019c00e943.json --skip-tflite`
  - The `--lineage` flag MUST be absolute and per-model. `emit_lineage` defaults to `build/lineage.json` and `preserve_metrics` only protects a re-export of the SAME checkpoint, so an export without it silently overwrites whatever model the manifest last described. Not hypothetical: the R1 run did exactly this to the incumbent's recorded metrics and its developer-phase release override, and only the surviving checkpoint made them recoverable
  - Step 3 — swap and deploy, one copy one build: `rm -rf MedataCore/Sources/Pipeline/Resources/segmenter.mlpackage && cp -R tools/segmenter/build/segmenter-0a019c00e943.mlpackage MedataCore/Sources/Pipeline/Resources/segmenter.mlpackage && make deploy-release`
  - Release is REQUIRED — Debug compiles `DEV_STUB_SEGMENTER` and loads `StubInferenceEngine`, which never touches the real model
  - Step 4 — verify the model and not just the build: `make deploy-release` prints `BUNDLED SEGMENTER:` and `DEPLOYED SEGMENTER:`, both of which must read `0a019c00e943`. Do NOT use the launch line for this — `make logs-device` reports only `stub` or `coreml` and never which model. The build stamp identifies the build, not the model inside it
  - Rollback needs no re-export and no retrain, because the incumbent artefact is still in `build/`: the same two commands with `segmenter-ab812dc3aa9d.mlpackage`
  - Export gates run automatically: the 24 MiB weight ceiling (Decision 13), 36 channels in palette order, PyTorch-oracle equivalence and runtime-preprocessing parity. Note `export_eligible` is false at the 0.48 bar for R3 — as it is for the shipped incumbent, which ships under the Decision 11 developer-phase override, so this is a known carry-over and not a new failure
  - ANE residency is the one bar that cannot be scripted: Xcode then Core ML performance report, on the M5 Pro Mac (this is `myfoodrepo-bridge` task 8). A model that converts cleanly can fall back to CPU silently and blow the 250 ms per view budget
  - Needs a decision entry recording the swap verdict in segmenter-foundation/decision_log.md, where the recipe verdicts live
  - Ready to run — the checkpoint exists and the recipe is adopted; what it needs is machine time and a human at the phone

- [ ] 12. STOP — R4/R5: the SegFormer-B0 longer-schedule pair, Decision 30's open question <!-- id:8l5zdrf -->
