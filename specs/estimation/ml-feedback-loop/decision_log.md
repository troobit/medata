...
## Decision 23: The triage ledger is a rolling, check-off-routed file; routing names no tooling
...
Non-estimation field notes were triaged into `cycles/cycle-<n>/triage.md`, written only when `field_close` ran. That made routing non-repeatable three ways: a note from a six-second notes-only pull was not routable until a full diagnose-and-close cycle happened; every cycle re-listed every non-meal note in the corpus, unchecked, under cycle-salted task ids, so nothing recorded that a note had already been filed; and the artifact sat inside the spec folder, which the external backlog-capture tooling refuses as a source by its own constraints. The feedback channel worked; the last hop into tracked development work did not, and the process must be executable by any agent harness without per-harness setup.

### Decision

Replace the per-cycle triage files with one rolling ledger, `specs/estimation/ml-feedback-loop/triage.md`, regenerated by a new `make field-triage` at any time and refreshed by `field_close`. Regeneration merges rather than rewrites: items are keyed by note id with cycle-independent task ids, and a checked item — checked off with a `routed: <destination>, <date>` detail — survives every regeneration as the durable routing record. The routing contract lives in this spec's design document; no skill, runner, or external tool is named or modified, and items reach the backlog workflow only as free-form text, never as a file handover.

### Rationale

The ledger's check-off state is the missing idempotence: an unchecked item is unrouted work, a checked item is a filed record, and both survive pulls, cycles, and regenerations. Keeping the contract as a self-contained repo file follows the cycle-file precedent (Decision 15) — fireability and state live in the file, so every harness gets the same interface — and leaving external tooling untouched means the process cannot drift when that tooling changes. Requirement 7.2's "per-pull triage artifact" wording was amended to match; the prior implementation did not satisfy it either (it was per-cycle over the whole corpus), so the amendment records reality rather than relaxing a met constraint.

### Alternatives Considered

- **Per-cycle files with carry-forward**: Keep `cycles/cycle-<n>/triage.md`, exclude note ids checked in earlier cycles - Rejected: routing state smeared across N files, an O(cycles) scan on every write, a moving "newest file" target for routing sessions, and routing still chained to cycle creation.
- **Amend the external backlog skill to read spec-folder triage files**: Give the capture tooling a carve-out and write-back duty - Rejected: grows a repo-specific wart in cross-project tooling the repo does not own, and couples the loop's repeatability to an external contract.
- **Routed-state in the corpus index** (`notes.routed_to` column): The index records routing, the file is a view - Rejected for now: the ledger must be reviewable and committable with the routing it records; a second state store adds a sync obligation for no capability. Mirroring into the index later for report metrics remains open.

### Consequences

**Positive:**
- A note is routable minutes after `make field-notes`; re-running any stage never re-offers filed work.
- One stable path and one state model for every agent harness; the file itself carries the interface.
- Git history of the ledger is the audit trail of what was routed where, per pull and per session.

**Negative:**
- The ledger grows monotonically (checked items are retained as the record); pruning, if ever needed, is a future decision.
- A generated, state-carrying file now lives beside authored spec documents and must be exempted from drift checks that assume spec folders hold only authored files.
- Meal-linked notes whose captures never become diagnosable (permanently missing bundles) still route through neither the overlay path nor this ledger; that gap is recorded, not solved.
