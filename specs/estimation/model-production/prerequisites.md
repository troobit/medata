...
- [x] **Acquire FoodSeg103** (Apache 2.0, https://xiongweiwu.github.io/foodseg103.html). Needed: the raw dataset on disk for the automated remap (`build_class_mapping.py`) and fixed-seed split (`prepare_dataset.py`). Record the source version / archive SHA so it can be written into `build/lineage.json` (`foodseg103_source`). Satisfies **Req 2.4**; feeds Req 1.3 lineage. *Done 2026-07-04:* on disk at `data/foodseg103/` in the release layout, all 7,118 pairs verified. Source of record is the HF parquet mirror (canonical zip host was down; mirror preferred by user decision) — provenance + SHA-256 release identifier in `data/foodseg103/SOURCE.md`, which is what `foodseg103_source` should carry. EXIF gotcha on 4 train images noted in `docs/agent-notes/dataset-strategy.md` §5.

## Training (Stage 3)

- [x] **Palette class-list lock — blocks the training run.** Before starting the GPU job, confirm the palette is locked at the final v1: `ClassPalette.v1Standard` redefined in place with the eight coarse liquid classes (water, coffee, tea, milk, fruit_juice, soup, beer, wine), giving 24 solid + 8 liquid + 3 special = 35 channels — the enumerated ordered list in `MedataCore/Sources/Segmentation/ClassPalette.swift` / `tools/food_db/generate.py` FOOD_DATA — and that the FoodSeg103 remap (`class_mapping_foodseg103_v1.json`) targets it. The trained checkpoint's output-channel count must match the shipped palette; training on the pre-liquid layout forces a full retrain. Recorded per `specs/estimation/nutrition5k-calibration/` Req 9.4 (Decisions 22–23 of that spec). *Verified 2026-07-04:* all three legs agree at v1/35 — `ClassPalette.v1Standard`, `generate.py` `verify_palette_lock` passes, and `class_mapping_foodseg103_v1.json` regenerated from the real `data/foodseg103/category_id.txt` (diff vs the committed artifact: one cosmetic source-name capitalisation, "French beans"; routing identical). `pytest tools/segmenter/tests` 44 passed / 1 torch-skipped; `make test` XCTest 364 (3 skipped, 0 failures) + swift-testing 114 passed.
