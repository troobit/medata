...
1. <a name="2.1"></a>The process SHALL remap FoodSeg103 (103 classes) to the 35-channel v1 palette via `tools/segmenter/build_class_mapping.py` and `class_mapping_foodseg103.json`.  
...
4. <a name="3.4"></a>IF the held-out mean IoU is below the re-derived gate (segmenter-foundation Decision 5, currently 0.48; was 0.60), THEN the checkpoint SHALL NOT proceed to export and the shortfall SHALL be recorded against the build.  
