...
- **Per-plate emission (Req 3.3/3.5/3.6/3.7):** overhead RGB (PNG), converted depth, pinned `nadir_intrinsics`, `gravity` (nadir → straight down), `ground_truth_class_mass_g` (mapped per §mapping), per-dish carb/protein/fat GT, `capture_path_canonical = "single_view_lidar"`, and the `estimator_path` stamp (routing below). Single-dominant fixtures additionally carry segmenter probs/argmax + the real checkpoint SHA.
