...
1. <a name="7.1"></a>The palette and food database SHALL define a documented set of standalone-liquid classes — reversing the v1 standalone-liquid out-of-scope decision (Decision 5, recorded and cross-referenced per [9.2](#9.2)) — each with a carbohydrate value and density sourced from CoFID (UK), AFCD (Australia), or USDA (US), with the source recorded per row.  
