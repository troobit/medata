...
- $\pi_{\text{sup}}$ is the support plane detected in the scene: **the surface the food rests on** — the plate top, board or tray where there is one, and the bare table only where the food rests directly on it. The earlier reading, "(plate / table)", is **superseded** by `support-plane-reference` Req 1.1: the two are not interchangeable. Where food sits on a raised support, fitting $\pi_{\text{sup}}$ to the surrounding table adds that support's height to every food pixel — measured at 26.1 mm, a 2–3.6× volume over-read, proportionally worst on the flattest food.
...
5. **There is a smallest food the depth path can resolve, and it is a bound on *width*, not on height.** ARKit's depth map is smoothed over roughly 4 pixels, which is a *lateral* footprint: smoothing preserves the amplitude of any plateau wider than its kernel and only widens the transition at its edges. So the limits are a **minimum food width of ≈ 8 mm at 350 mm capture range** — narrower food is amplitude-attenuated and genuinely unrecoverable — and a **minimum food height of ≈ 2–4 mm**, bounded by depth noise and support-plane fit error rather than by lateral smear. Both scale with range: the width floor is ≈ 6 mm at 300 mm and ≈ 10 mm at 500 mm, so the envelope, not the bare constant, is the statement. A 3 mm-tall item 100 mm across occupies ~2,700 depth samples and survives intact. Food below the width floor **falls back to the edge-band reference rather than being silently under-measured** ([4.2](#4.2)). The residual is a perimeter-proportional under-read of a border strip about half a kernel wide, therefore worst on small items. (`support-plane-reference` Req 2.5. An earlier draft of that spec stated an 8 mm *height* floor; it had the axis the wrong way round and is superseded.)
6. **Single-view path silently undercounts inter-class occlusion.** When a tall food occludes a shorter food in the nadir view (rice partly behind a chicken breast), the height-field integration in the single-view path produces zero volume for the occluded portion. The two-view path partially recovers occluded food via the oblique silhouette. The single-view path's geometric-completeness sub-confidence (Req 13.2) is reduced when inter-class occlusion is detected, and the user is prompted to recapture using the two-view path.
...
**User Story:** As the system, I need the support plane — the surface the food rests on — in the scene's metric coordinate frame, so that voxel carving has a lower bound and the visual hull can be closed from below.
...
2. <a name="4.2"></a>WHERE LiDAR depth is available, $\pi_{\text{sup}}$ SHALL be fit to **the surface the food rests on**, by RANSAC over depth points enumerated on the **native depth grid** within a millimetre-denominated annulus around the food region mask, and a fitted candidate SHALL be admitted only where a contact ring immediately outside the food boundary supports it in aggregate, across radial bands, and across angular sectors. WHEN no candidate is admitted, the system SHALL fall back to a RANSAC fit over depth points at and around the lower edge of the food bounding region, and SHALL record which of the two references produced the plane. **The edge-band scan is the fallback, not the fit.** The earlier form of this criterion named it as the only fit; that form is **superseded** by `support-plane-reference` Reqs 2–4, because edge-band sampling fits the surrounding table rather than the support, and integrating above the table adds the support's height to every food pixel.
3. <a name="4.3"></a>WHERE LiDAR depth is unavailable but the canonical two-view path is in use AND a card has been detected per [5.2](#5.2), $\pi_{\text{sup}}$ SHALL be recovered as follows: (a) initialise the metric scale at the *card plane* using $s_{\text{card,init}}$ from the recovered card pose ([5.3](#5.3) initial value); (b) back-project the lower silhouette edges across both views using $s_{\text{card,init}}$ and the gravity vector to obtain a candidate $\pi_{\text{sup}}^{(0)}$; (c) iterate by lifting $s_{\text{card}}$ from the card plane to the food plane (offset by the food's mean height above $\pi_{\text{sup}}^{(k)}$) and re-fitting $\pi_{\text{sup}}^{(k+1)}$ until $\|\pi_{\text{sup}}^{(k+1)} - \pi_{\text{sup}}^{(k)}\| < 1$ mm or 5 iterations are exhausted. Convergence SHALL be persisted as a sub-confidence signal. The amended *definition* of $\pi_{\text{sup}}$ in [4.2](#4.2) and the glossary applies to this path unchanged in meaning, but this path's **behaviour does not change**, because it derives no plane from depth: the silhouette lower edges it back-projects already lie on the surface the food rests on, so it was never fitting the surrounding table. This path therefore records no support-plane reference and takes no fallback (`support-plane-reference` Req 1.5).
