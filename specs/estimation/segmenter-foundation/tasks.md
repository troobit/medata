...
- [x] 22. STOP — IF the spike passes, retrain SegFormer-B0 and compare with the adoption margin <!-- id:2mfkxz0 -->
  ...
  - 2026-08-13 VERDICT — REJECTED, `deeplab_mnv3` stays (Decision 30). Candidate `89f305fa30dc` (segformer_b0, 12 epochs, 6h19m MPS) vs incumbent `ab812dc3aa9d` on the leak-free anchor: mean food-class IoU 0.2912 against 0.3927 (-0.1015) and staple mean 0.2987 against 0.3870 (-0.0884), where adoption needed >= +0.02 on BOTH. Merged val agrees as context: 0.3716 against 0.4420 (-0.0704) and staple 0.3632 against 0.4503 (-0.0871). Recipe parity verified from both lineages (same 45,515/1,711 split, CE loss, 12 epochs, lr 1e-3, poly-0.9, 36 classes, 513x513) — `arch` is the only recorded difference. Scope: rejected AT EQUAL STEP BUDGET; the candidate was still improving at epoch 12 (+0.0116) while the incumbent had plateaued, so a longer-schedule rematch is unanswered rather than closed and would need two gated runs for attributability.
