...
**Superseded on the cone (Decisions 52, 54, 55).** It is reused; it is not correct. `gravityAngleMaxRad` is enforced on the two hypothesis tests and *not* on the refinement between hypothesis and polish, and the polish gate's rejection path is `break`, which keeps the plane it rejected the successor of. Measured over a 1…90° sweep the candidate set is outside the cone at every value that admits one — 4 of 4 candidates outside a 1° cone, 5 of 6 outside 2° with the worst at 18.955°, one of six outside the shipped 15° at 20.512° — so the violation grows as the bar falls and no value repairs it. Read every claim below stated "inside the cone" or "capped at 15°" as a claim about a bound the shipped candidate set does not respect.
...
**Settled, and the second review's premise is withdrawn (Decisions 52, 55).** Measured at the shipped band, the plate candidate's largest connected component holds **22.1 %** of its members within one band of the surface below it, so the straddler is real and the first review's reading is the one that holds. The second review's arithmetic is sound and its premise is not: the candidate set is not 15°-capped (Decision 55), and the corpus's own extraction produces planes at 20.512° and, with the gate switched off, 79.809°. Do not restate the refutation with a wider cone either — it is the *enforcement* that is missing, not the angle.
...
> **"One constant" is superseded** (Decision 56): it is **two**, and the second one is the p90. `foodEnvelopePercentile = 0.90` is the *denominator* of `foodEnvelopeMinMm` and carries no derivation of its own — the `[derived] Decision 22` marker derives the guard's **kind**, not its percentile. Swept over its domain the corpus ceiling on the bar moves **45.453 mm**, so every bound quoted on `foodEnvelopeMinMm` anywhere in this feature is a reading at 0.90. The percentile is `[owed]`, bracketed 0.1…0.92, and must be fixed **before** the bar. The three cases above still hold at every value in that bracket; what does not hold is that one number settles them.
...
> **"Nothing in it moves when task 26 sets the `[owed]` constants" is superseded** (Decision 57). It holds for every owed constant in `SupportRegion`, because the named planes this suite measures are chosen before `admissibility` runs. It does **not** hold for `LiDARPlaneFitter.maxIterations`, which is `[owed]` as of Decision 57 and is what *produces* the pre-feature plane: below a budget of 4 all three parity-capture assertions go red at once — ring median −4.309 mm (the sign Req 6.2 exists to test), support fraction 0.594 against a 0.5 bar, volume 200.885 cm³ against 682.96. So this suite is a **second bracket** on that constant, in the way Decision 41's scene suites are on six `SupportRegion` ones, and it is the first time the committed suite reads the **fallback** leg at all. It is also the **looser** of the two sources — it floors the budget at 4 where the corpus floors it at 8 — so a value passing every assertion here can still sit 2.231 mm from the shipped plane, past Req 5.1's 1 mm. These are volume and ring-median bands, not transfer bands.
