...
| Depth grid ≠ 256×192 (N5k identity grid) | mm-based radii transfer (Req 5.1) — measured at ≤ 1 mm across a 2× halving. `minAcceptedExtentPx` was the one bar that did **not** transfer (Decision 35); it is now `minAcceptedExtentMm` and does, with zero verdict flips across the same halving (Decision 37) |
