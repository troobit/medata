...
1. <a name="5.1"></a>Given the same capture inputs, the device and the offline replay SHALL produce support-plane coefficients equal within a documented tolerance, verified on a named fixture. **Both are now measured (Decision 35): the tolerance is 1 mm of plane movement at the food, and the named fixture is `1785135663727`.** On identical bytes the two paths share one implementation and agree exactly; what differs is the depth grid, and across a 2× halving the plane moves **0.835 mm** on that fixture and **0.037 mm** on `1785901032716`. The transfer holds down to 128×96 and refuses at 64×48, where the ring's inner band falls to 37 samples against a 200 floor. One bar did not transfer — `minAcceptedExtentPx` was denominated in pixels, so a surface admitted at 44 px was rejected at 22 px — and **Decision 37 closes it**: the bar is now `minAcceptedExtentMm = 44`, the value unchanged in physical terms, and the same halving produces zero verdict flips. No bar in `admissibility` is denominated in pixels. The device leg of this criterion remains task 27's.  
...
2. <a name="6.2"></a>Replaying capture `1785135663727` under the pre-feature fit SHALL yield a ring measure in the +18…+26 mm range recorded by the diagnosis, and under the corrected fit SHALL yield approximately zero. This makes the diagnostic falsifiable rather than merely recorded. **The +18…+26 mm range is superseded by Decision 28**: measured against the committed depth slice it is **+4.2 mm** — the sign and the separation hold, the magnitude does not. The corrected measure holds at **−0.93 mm**. `SupportPlaneRegressionSliceTests` asserts the separation.  
...
1. <a name="7.1"></a>Replaying capture `1785135663727` SHALL yield a food volume within 5 % of 235.96 cm³, the value the restricted fit produced during diagnosis, against 682.96 cm³ under the current fit. This is a parity check against a known implementation, not an accuracy claim — that capture has no weighed truth. **The 235.96 cm³ target is superseded by Decision 28**: the pre-feature volume reproduces at **714.8 cm³** (+4.7 %, inside this criterion's own band), but the corrected volume measures **306.8 cm³**, 30 % above the stated figure, and the known implementation it was a parity check against was deleted in task 23. A reduction of more than half is asserted in its place.  
2. <a name="7.2"></a>Replaying capture `1785901032716` (2 slices of bread, weighed at 80 g) SHALL yield a food **volume** within 20 % of 200 cm³, against 714.84 cm³ today. The criterion is stated in volume because mass depends on the class and density this feature does not control, and the same scene classified as both `bread_wholemeal` and `carrot` 30 s apart. The class SHALL be pinned as a test precondition. The 20 % band is the sum of the accepted overhang under-measure (~11 %, Decision 4) and the measured device-to-replay divergence (~5 %), rounded up. **The ~200 cm³ target is unreachable and is superseded by Decision 28**: the pre-feature volume reproduces at **735.7 cm³** (+2.9 %), but no candidate plane yields under 646.7 cm³, because the constraint is the mask rather than the plane — it covers ~298 cm² where two slices of bread are ~200 cm², and 200 cm³ over 298 cm² implies a 6.7 mm mean food height against the ~11 mm the depth map shows. The gap is segmentation, which this feature does not control.  
...
**Resolved for Reqs 6.2, 7.1 and 7.2 (task 22).** `tools/fixture_slice.py` cuts a ~290 KB depth-only slice — the native 256×192 Float32 depth map, its confidence bytes, and the food mask reduced onto the same grid — out of each bundle. Both slices are committed under `MedataCore/Tests/SupportPlaneTests/Fixtures/`, so those three criteria now run from a clean checkout with no device. 199 MB of the 204 MB bundle is the segmentation probability tensor, which the support-plane fit never reads.

**Reqs 7.3 and 7.4 remain blocked, and on captures rather than on code.** Req 7.3 needs a weighed non-flat capture; the 208 g rice bundle it originally named cannot serve, because `FixtureLoader` loads zero meals from it (`prerequisites.md`, capture 2 is the replacement). Req 7.4 needs a bowl capture with in-palette food; the 2026-08-05 attempt used prawns, which are not among the 25 solid classes, so it refused at segmentation and never reached plane selection (`prerequisites.md`, capture 5). Neither capture exists, so neither criterion is executable at any cost in code, and no slice can be cut for them.
