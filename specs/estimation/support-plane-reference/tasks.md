...
- [ ] 26. Determine the constants the design deliberately left unstated <!-- id:284ca5r -->
  ...
  - supportVisibilityMin = 0.15 appears unfirable at the stated formula, found while implementing tasks 7-8. For a visible support strip w px wide around food of radius f px the ratio is ~2w/f, so 0.15 needs w < 0.075f — and a strip that thin sits entirely inside the 8mm ring inner radius, so the ring lands beyond it and the guard never sees the case. The fully-covered-well scene falls back on the food-envelope guard instead. Measure w/f across the corpus before fixing the constant, or the Req 3.9 guard is decorative
  - The window in which the aggregate passes and sectors fail is narrow with the placeholder sector trio: supporting sectors for a contiguous supported arc land within +/-1 of fraction x ringSectorCount, so ringSupportMin = 0.6 against minSupportingSectors = 6 of 8 leaves only fraction in ~[0.60, 0.75]. Decision 18's 0.65 case sits inside it but near the edge. Measure whether the corpus's edge-crossing captures fall in that window, or raise sectorSupportMin / minSupportingSectors with the evidence recorded
