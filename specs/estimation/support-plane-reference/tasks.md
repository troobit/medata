...
- [ ] 26. Determine the constants the design deliberately left unstated <!-- id:284ca5r -->
  ...
  - SETTLED (Decision 29): ringInnerMm = 8 holds at corpus range — the 4px smear is 7.45mm and 7.36mm at 338.9mm and 336.9mm — with a stated envelope of ~365mm, beyond which it must become max(ringInnerMm, 4 x mmPerPx). Closes the prerequisites item of the same name
  ...
  - BRACKETED (Decision 32): minAcceptedExtentPx sits in 13...26 on the corpus — above a 153-sample 12px sliver it correctly rejects, at or below the 26px smallest extent that reaches the later guards. The shipped 24 has 2px of margin, asserted so a capture that narrows it fails the test rather than silently losing a candidate. The VALUE is still owed
