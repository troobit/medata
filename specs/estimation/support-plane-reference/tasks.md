...
- [ ] 26. Determine the constants the design deliberately left unstated <!-- id:284ca5r -->
  ...
  - REMAINING and gated: ringSupportMin, the sector trio plus maxCrossedSectors (Decision 40; ringSectorCount bracketed 4...8 per Decision 44, sectorSupportMin bracketed 0...0.5 per Decision 45 and ringOuterMm bracketed 22...32mm per Decision 46, the three to be fixed JOINTLY and BEFORE the sitting, radius first; ringBandCount bracketed 2...3 per Decision 47 and settled by committed evidence rather than by capture), ringSupportMarginMin, ringOuterMm (bracketed but NOT interpolable — Decision 46 replaces the rule Decision 33 found unsatisfiable), bandStepMaxMm (denominated in ringBandCount, Decision 47), foodEnvelopeMinMm, minResidueAreaMm2 (floor only — the denomination is settled, Decision 38), minAcceptedExtentMm (value only — the denomination is settled, Decision 37), supportVisibilityMin (value), Req 4.5's fallback-rate threshold, fallbackPenalty (value — now the SAME gate as the rate, Decision 36). Req 4.5's denominator does not exist until model-production Bucket C: N5k pre-checkpoint ingestion runs no segmenter, so those 3,490 fixtures carry no food mask and fitFoodSupportPlane has no input on them
