...
|---|---|---|
| Graph (root, as-built Decision 20) | `MealRoute` | Day-list rows push `.overview` (Req 10.6) *(Superseded by `ui/home-router` Decision 16: day-list rows push `.result` directly)*. Its toolbar presents the Capture / Data / Settings covers |
...
| Meal overview (§9) | new `MealOverviewView.swift` | N | Photo via extracted `PHAsset` loader; masks via `MaskOverlayLoader`; corrections re-read on `eventsDidChange` while visible; delete via confirmation *(Superseded by `ui/home-router` Decision 16: the page is deleted; its photo/metadata/delete content lives on `ResultView`)* |
