...
| Piece | Location | Role |
|---|---|---|
| `GlucoseWidgetShared` (new SwiftPM target + its own library product, Foundation-only) | `MedataCore/Sources/GlucoseWidgetShared/` | Snapshot DTO, `GlucoseTrend`/`GlucoseBandStatus` enums, App Group id, `GlucoseSnapshotStore` read/write. A **discrete linkable product** so the extension links exactly this and cannot pull GRDB via an umbrella product; no third-party deps. |
...
| `GlucoseWidget` + provider/entry/views | `MeData/MeDataWidgets/GlucoseWidget.swift` (new file in the existing target) | The data-driven widget kind; reads the snapshot only. |
...
**Value sanity guard (Req 2.7).** No source in the codebase emits an out-of-range HI/LO sentinel — `bsl` values are plain mmol/L `Double`s from CGM ingest and Libre OCR — so 2.7 is vacuously satisfied today. As a defensive net, the snapshot builder maps a non-finite or implausible-range `mmolL` (outside ~1.0–35.0) to `.neverRecorded` rather than rendering a fabricated decimal, so a future sentinel-emitting source cannot silently display a fake number.
...
### Widget provider, entry, timeline (pure entry generation — Req 5.4)

```swift
struct GlucoseEntry: TimelineEntry { let date: Date; let render: GlucoseRender }

// Pure, unit-tested: maps (snapshot, referenceDate) → the render state at that instant.
enum GlucoseRender: Equatable {
...
enum GlucoseTimeline {
    // Entry instants are anchored to readingDate (the staleness boundaries are ages of
    ...
    // stale (or never-recorded) at `reference`, a single entry is emitted.
    static func entries(_ snapshot: GlucoseSnapshot, from reference: Date) -> [GlucoseEntry]
    static func render(_ snapshot: GlucoseSnapshot, at date: Date) -> GlucoseRender
}
```
...
Add to `handleDeepLink`:

```swift
case ("graph", ""), ("graph", "/"):
    // same sequencing as the capture case: dismiss any presented cover/sheet, then present .graph
```
...
| `mmolL` non-finite or outside ~1.0–35.0 | Snapshot builder emits `.neverRecorded` (no fabricated decimal) | 2.7 |
...
- **Dependency-graph assertion** (in `GlucoseWidgetSharedTests`, mirroring the cgm-connect firewall approach): `swift package dump-package` shows `GlucoseWidgetShared` has an empty dependency list, so the extension link closure cannot acquire GRDB/Persistence.
- **`GlucoseTimeline.render`/`entries`**: given a snapshot at `T`, `render` returns fresh / stale / lastReading / neverRecorded across the 15- and 30-minute boundaries; `entries` emits the three boundary entries in order; age-string formatting incl. the future-timestamp clamp.
...
| 2.1–2.8 | `GlucoseWidget` kind + per-family views; functional copy; mmol/L 1-dp; 2.7 vacuously satisfied (no upstream HI/LO) + sanity guard; locked-render intent |
...
| 5.1–5.5 | `GlucoseTimeline.render`/`entries`; opacity de-emphasis; last-reading vs never-recorded; age format + skew clamp |
| 6.1–6.2 | Publisher `reloadTimelines(ofKind:)` on change; prime write on init; no widget-side polling; per-state reload policy |
| 7.1–7.2 | `medata://graph` route + `widgetURL`; locked-open defers to unlock |
