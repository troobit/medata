...
1. <a name="3.1"></a>The system SHALL derive the trend from `bsl` readings whose timestamps fall within the 30 minutes preceding the current time — the closed window `[now − 30m, now]`, both bounds inclusive — as a single rate of change in mmol/L per minute over those readings. *(Widened from 15 minutes by Decision 15 after the live LibreLinkUp cadence was measured at a modal 15-minute gap. Narrowed to a single-provenance series by `specs/data/fingerprick-glucose` [Req 5.1](../../data/fingerprick-glucose/requirements.md#5.1): sensor readings when they satisfy Req 3.3, otherwise blood readings alone under the same rules, never a mixed series. The extension's own fetch holds only vendor sensor data, so its derivation is unchanged.)*  
