...
- [ ] 16. Widget falls out of sync while the app is suspended <!-- id:7k39hjq -->
  ...
  - [ ] 16.11. STOP — device confirmation of the monotonic guard, plus the StandBy colour check this task owes
    ...
    - `make test` re-run 2026-08-28 off-device, now that 751763f fixed the food-DB bake's PEP 604 import failure: XCTest 653 executed / 3 skipped / 0 failures; swift-testing 479 tests in 54 suites, all passed (2302.8s under this run's machine load). `make spell` clean. Both totals green with no PATH prefix needed — the bake now imports under the bare system interpreter, which was the whole point of that fix. This closes the agent-completable half of this subtask's own gate; the two device criteria above are unaffected and still open
