...
3. <a name="3.3"></a>WHEN the user taps a meal row, THEN the system SHALL present that meal's full detail on a single screen — photo, per-food rows carrying the serving-adjustment surface, corrected totals, capture metadata, delete, and the dose readout (`specs/data/insulin-dosing` [Req 6.10](../../data/insulin-dosing/requirements.md#6.10)) — with no intermediate summary screen between the row and that detail; insulin and glucose rows SHALL NOT navigate to a detail view. *(Redefined in place by Decision 16. Superseded wording: "WHEN the user taps a meal row, THEN the system SHALL present that meal's existing detail view; insulin and glucose rows SHALL NOT navigate to a detail view.")* *("recorded dose readout" reworded per `specs/data/insulin-dosing` Decision 18: the readout recomputes live; nothing derived is stored.)*  
...
8. <a name="4.8"></a>The reading SHALL act as a route to Graph — tapping it SHALL present the Graph surface — and SHALL NOT be editable or an entry point; glucose entry belongs to the BSL control (`specs/data/fingerprick-glucose` [Req 2.1](../../data/fingerprick-glucose/requirements.md#2.1)). *(Redefined in place by `specs/data/fingerprick-glucose` Decision 5. Superseded wording: "The reading SHALL be display-only — not tappable, not editable, and not a route.")*    
