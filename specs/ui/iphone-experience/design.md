...
- `liveDistanceCm` ← near-side 10th-percentile of `frame.sceneDepth?.depthMap` over a centre-window crop — `nil` when `sceneDepth` is unavailable. NOT the median: in a top-down capture the food is the closest surface in the crop, and a loosely-framed plate can put the farther surrounding table over half the crop, where a median reports the table's distance instead (bugfix task 61; the pipeline's support-plane fitter documents the identical two-surface-mixture failure at `specs/estimation/pipeline/design.md` §6.2.1).
...
- **`LiveSampleObserver` distance + coverage maths** — unit tests with synthesised `CVPixelBuffer` depth/confidence pairs (existing fixtures in `CaptureKitTests`); assert centre-crop near-side percentile and `≥ τ_conf` count.
...
- **Refusal banner** — XCUITest: trip `EstimationFailure.noScaleAvailable` via a fixture, assert banner appears with the refusal message, "Try again" returns to `.capturing` at the right stage.
