...
- **Per-stage Δθ confidence fields** — `PbConfidenceResult` does not yet carry `deltaThetaNadirDeg` / `deltaThetaObliqueDeg`. Until a research-side spec wires them through `PortableContracts`, `ResultView`'s §9.3 very-low-confidence surface renders the angular error as `0°` (the explanation copy and Retake / Keep-as-is controls are unaffected). See `decision_log.md` Decision 21.
