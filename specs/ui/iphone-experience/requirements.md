...
**Supersession note (2026-07-04):** Parts of this spec are amended or superseded by [`specs/ui/design-handoff-00/`](../design-handoff-00/). The binding per-requirement status is in the [§16 supersession table](../design-handoff-00/requirements.md#16-supersession-of-iphone-experience) of that spec. All changes are in the same commit series.
...
3. <a name="9.3"></a>WHEN `σ_meal < 0.20`, THEN the result view SHALL display an inline explanation that the estimate may be wrong by orders of magnitude AND SHALL surface the per-stage angular error Δθ that contributed to the low confidence, alongside a "Retake" control and a "Keep as-is" control (research Req 13.5; the "manually correct" branch is out of scope per the Non-Goals section). **Known limitation (best-effort enforcement):** the persisted `PbConfidenceResult` does not yet carry `deltaThetaNadirDeg` / `deltaThetaObliqueDeg`, so `ResultView` renders the Δθ as `0°` until a sibling research-side spec wires those fields through `PortableContracts`. The inline-explanation and Retake / Keep-as-is controls are fully satisfied; only the numeric Δθ value is stubbed. Tracked in `decision_log.md` Decision 21 and `prerequisites.md` (Out of band — sibling specs).
...
5. <a name="9.5"></a>The result view SHALL NOT display per-class breakdown, clinical macros (energy, protein, fat, fibre), or any persistence-layer fields beyond the carb total and the quality pill. *(Redefined in place 2026-08-28 by `specs/data/insulin-dosing` Decision 17: where a dose exists the pill holds the dose and carries confidence as its fill; where none exists it is the confidence pill as before. Superseded wording: "beyond the carb total and confidence pill".)*  
...
4. <a name="19.4"></a>WHEN the user taps a row, THEN the system SHALL push a meal-detail view onto the Meals tab's navigation stack. The detail view SHALL reuse `ResultView` and SHALL display the same content the user saw immediately after the original capture (the quality pill, carbohydrate total, photo thumbnail, placeholder banner where applicable). *(Redefined in place 2026-08-28 by `specs/data/insulin-dosing` Decision 17 — the trailing pill now holds the dose where one exists. "The same content the user saw" is the load-bearing clause and still holds: both surfaces changed together. Superseded wording: "(confidence pill, carbohydrate total, …)".)*
...
8. <a name="20.8"></a>The ResultView SHALL render the carbohydrate total at the `display` type-scale (72pt heavy monospaced) centred over a dimmed full-bleed photo background, with the quality pill immediately below and (where applicable) the placeholder chip from research Req §23.3 as a small pill — NOT a full-width yellow banner. *(Redefined in place 2026-08-28 by `specs/data/insulin-dosing` Decision 17: the pill in that position holds the dose, filled with the confidence tier's colour, falling back to the named confidence pill for a meal with no dose. Position, metrics and the not-a-banner rule are unchanged — the replacement is deliberately geometry-identical. Superseded wording: "with the confidence pill immediately below".)*
...
10. <a name="20.10"></a>All interactive controls SHOULD meet a ≥48pt touch target (use `hitSlop` when the visual size is smaller) — **NON-BLOCKING ([MD-32](../../DECISIONS.md#md-32-accessibility-is-never-a-blocking-gate))**. Press feedback SHALL appear within 100ms of touch-down (this half remains blocking). These rules apply uniformly across the tab bar, the shutter, indicator chips, capture-mode pill, list rows, and toolbar buttons.
11. <a name="20.11"></a>The implementation SHOULD respect `accessibilityReduceMotion`: spring animations replaced by a single crossfade; the carb-total `contentTransition(.numericText())` falling back to a snap-in. **NON-BLOCKING ([MD-32](../../DECISIONS.md#md-32-accessibility-is-never-a-blocking-gate))** — shipped conformance stays.
12. <a name="20.12"></a>The implementation SHOULD respect Dynamic Type up to size `AX5`; the carb total's `display` style SHOULD clamp at 88pt to prevent the value running off-screen. **NON-BLOCKING ([MD-32](../../DECISIONS.md#md-32-accessibility-is-never-a-blocking-gate))** — shipped conformance stays.  
