...
**That work is deferred to an estimation-domain spec** (Decision 18). This spec orders the shortlist by **recency** — foods the user has recently chosen for a food of that kind — and relies on the full eligible list for everything else. On a 25-item universe where the five-item shortlist already covers a fifth of the candidates, ordering is a convenience rather than a functional gate; the design was already conceding recency-only for liquids and for replayed fixtures, which made score ordering a partial answer at best.
