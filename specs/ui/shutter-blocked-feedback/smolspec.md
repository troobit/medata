...
- The system MUST keep the shutter's disabled state discoverable to VoiceOver now that the `.disabled()` modifier has been removed. Because `AccessibilityTraits.isNotEnabled` is not a public SwiftUI API (see Decision 5), the existing `.accessibilityValue(state.accessibilityValue)` — which announces "Disabled" / "Capturing" / "Ready" — carries the disabled announcement. The existing `accessibilityLabel`, `accessibilityHint`, `accessibilityValue`, and `accessibilityIdentifier` SHALL be unchanged.
