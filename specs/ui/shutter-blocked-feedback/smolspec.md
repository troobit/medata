...
- The system SHOULD keep the shutter's disabled state discoverable to VoiceOver now that the `.disabled()` modifier has been removed. Because `AccessibilityTraits.isNotEnabled` is not a public SwiftUI API (see Decision 5), the existing `.accessibilityValue(state.accessibilityValue)` — which announces "Disabled" / "Capturing" / "Ready" — carries the disabled announcement. **NON-BLOCKING ([MD-32](../../DECISIONS.md#md-32-accessibility-is-never-a-blocking-gate))** for the announcement itself. The existing `accessibilityLabel`, `accessibilityHint`, `accessibilityValue`, and `accessibilityIdentifier` SHALL be unchanged — that clause stays **blocking**, because it forbids a regression of what already ships rather than demanding new conformance.
...
- `App/ShutterButton.swift`
  ...
  - Do not add an explicit not-enabled trait: `AccessibilityTraits.isNotEnabled` is not a public SwiftUI API (Decision 5). The existing `.accessibilityValue(state.accessibilityValue)` already announces "Disabled" to VoiceOver, which is the announcement `.disabled()` was supplying.
...
- **Risk: Removing `.disabled()` changes VoiceOver behaviour.** Mitigation: the existing `.accessibilityValue(state.accessibilityValue)` announces "Disabled" / "Capturing" / "Ready" (an explicit `.isNotEnabled` trait was intended but is not a public SwiftUI API — see Decision 5). Validate the spoken value on device with VoiceOver before merging.
