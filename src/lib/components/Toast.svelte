<script lang="ts">
	/**
	 * Toast notification component.
	 * Req 4.6: Visual confirmation on save
	 * Req 9.7: Direct messages without apologies
	 */

	interface Props {
		message: string;
		type?: 'success' | 'error' | 'info';
		duration?: number;
		onDismiss?: () => void;
	}

	let { message, type = 'success', duration = 3000, onDismiss }: Props = $props();

	let visible = $state(true);

	// Auto-dismiss after duration
	$effect(() => {
		if (duration > 0) {
			const timer = setTimeout(() => {
				visible = false;
				onDismiss?.();
			}, duration);
			return () => clearTimeout(timer);
		}
	});

	function handleDismiss() {
		visible = false;
		onDismiss?.();
	}

	const typeStyles = {
		success: 'bg-brand-accent text-black',
		error: 'bg-red-600 text-white',
		info: 'bg-white/10 text-white'
	};

	const typeIcon = {
		success: 'M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z',
		error: 'M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z',
		info: 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z'
	};
</script>

{#if visible}
	<div
		class="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 animate-slide-up"
		role="alert"
		aria-live="polite"
	>
		<div class="flex items-center gap-2 rounded-lg px-4 py-3 shadow-lg {typeStyles[type]}">
			<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
				<path fill-rule="evenodd" d={typeIcon[type]} clip-rule="evenodd" />
			</svg>
			<span class="font-medium">{message}</span>
			<button
				onclick={handleDismiss}
				class="ml-2 -mr-1 p-1 rounded hover:opacity-70 min-w-[44px] min-h-[44px] flex items-center justify-center"
				aria-label="Dismiss"
			>
				<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
					<path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
				</svg>
			</button>
		</div>
	</div>
{/if}

<style>
	@keyframes slide-up {
		from {
			opacity: 0;
			transform: translate(-50%, 20px);
		}
		to {
			opacity: 1;
			transform: translate(-50%, 0);
		}
	}

	.animate-slide-up {
		animation: slide-up 0.2s ease-out;
	}
</style>
