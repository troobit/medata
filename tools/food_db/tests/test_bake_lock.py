...
match ``ClassPalette.version`` (the Swift single source of truth, currently 'v1').
...
def test_class_palette_version_reads_v1_from_swift():
    # Read straight from ClassPalette.swift's v1Standard so the lock tracks the
    # Swift source, not a duplicated constant.
    assert generate.class_palette_version() == "v1"
...
def test_lock_fails_on_mismatch():
    with pytest.raises(SystemExit):
        generate.verify_palette_lock("v2-does-not-match")
