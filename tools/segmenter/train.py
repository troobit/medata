...
def train(args) -> int:
    ...
    _save_checkpoint(model, args, best_miou)
    return 0


def _save_checkpoint(model, args, last_miou: float) -> None:
    ...
    keys are provenance (§11) for the iteration loop / fixture stamping.
    """
    torch = _import_torch()
    out = Path(args.out)
